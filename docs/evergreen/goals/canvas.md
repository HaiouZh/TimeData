...
点节点只选中，显式动作才写入或导航：Task 可就地完成/取消完成；单 Goal 成员可确认后移出；同 Goal 内成员可用“连前置”建立 blocker -> blocked 前置，进入连前置草稿态后关闭动作单，并在画布顶部提示“点击目标节点完成连接，点空白处取消”与“取消”按钮；草稿态点到恒星不静默，给出“Goal 锚不参与前置”的解释并保留草稿，用户可继续点成员；写入成功在画布右下角给一次确认提示（3 秒自消），与错误红条、灵动模式提示共用同一提示位，优先级为错误 > 确认 > 灵动提示；选择前置边可删除；恒星可打开添加成员 picker，也可进入目标设置 sheet 编辑、归档或确认删除 Goal。所有结构写入仍只经 `lib/goals.ts`、`lib/tasks.ts` 与 `lib/goalLayoutPins.ts` 的既有 Dexie + `syncLog` 边界，写入经 `recordSyncLog` 自动调度上传。桥接节点的原子动作（如 Task 完成）就地生效；“移除成员 / 连前置”这类需要判定“哪个 Goal”的操作会先让用户选择 Goal，再导航到对应 `/goals/:id` 聚焦编辑器处理。
