...
6. Router 注册时间轴、速记、待办、轨道、目标、时间/健康统计、设置及记录编辑路由；AppShell 页面壳与底部导航样式消费 design-language 的 `page/surface/border/ink/accent` token，结构和导航语义不变；目标详情 `/goals/:id` 与轨道详情 `/tracks/:id` 一样隐藏底部导航；设置子路由包含底部导航、轨道行动标签、统计布局、健康范围、服务端/数据/管理等入口，具体归属见各主题文档。
