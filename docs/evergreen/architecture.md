...
```
用户操作 → TodoPage / TaskDetailSheet / RecurrencePresetSheet
        → Dexie.put(tasks)
...
关键点：`tasks` 是轻量待办域，不引用分类或时间记录。普通任务池条目用 `done` 表示完成，并用 `completedAt` 记录完成时间；重复任务用 `recurrence` + `startAt` + `lastDoneAt` + `completedCount` 计算当前是否到期和是否已耗尽。`turn/turnAt` 是可选回合轴，表达"等我 / 在跑 / 搁置"的当前状态和进入时间；`tags` 是自由标签数组，只供人/agent 表达语义，不驱动自动逻辑。完成仍以 `done=true` 为唯一真相。无终止条件的重复任务勾选后继续循环，同时把 `subtasks[].done` 重置为 `false` 让下一轮重新就绪；带 `count` 或 `until` 的重复任务在做满/无后续发生后置为 `done=true` 并沉入"已完成"区（终结性完成保留子任务勾选）。待办首页四分区：**今天 / 已完成 / 收件箱 / 已排期**（已排期 = 一次性未来排期 ∪ 未到期重复，按下一发生日升序）；列表展示复选框、根任务名、必要元信息和可展开子任务大纲；子任务可在列表内勾选、编辑和新增，也可在详情抽屉编辑，二者共用同一套草稿/提交语义。宽窄屏统一只走 **swipe + 详情抽屉**：行内不再渲染移动/删除/日期编辑按钮，所有日期/重复编辑都进 `TaskDetailSheet` → `RecurrencePresetSheet` → `CustomRecurrencePage`。重复设置走"徽章 → 预设门 → 自定义整页"：常用的每天、工作日、每周、每月和月末一击写入，复杂规则进入自定义页；已有多周几、多月号和 `byMonthday: [-1]` 月末规则不会因打开/完成而静默降级。非重复排期任务过期后回到收件箱不堆进今天；重复任务过期在"今天"区以红色"逾期 M月D日"标签呈现（由 `currentDueDateString` 派生）。服务端暴露只读 `GET /api/tasks` 供受控客户端查询；Web 端写入仍先落 Dexie，再经同步管线送到服务器；授权 agent 的状态回写走 `/api/agent/tasks/:id/status` 的封闭动作集合。
...
外部 agent / CLI task-handback
        ...
        → server 读当前 task，按封闭动作更新 turn/turnAt/done/subtasks/tags
...
关键点：`AGENT_TOKEN` 只在 `/api/agent/*` 生效；泄露影响面限制在任务回合状态、完成标记、备注子任务和 tags，不授予 sync、force-push、admin、export 或 reset 权限。CLI 的 `task-running` / `task-handback` / `task-park` / `task-done` / `task-tag` 只是这个受控 API 的简化封装。
...
1. **跨端契约只在 `packages/shared`**：`packages/shared/src/types.ts` 导出 `Category`、`TimeEntry`、`QuickNote`、`Task`、`Recurrence`、`Setting`、`SyncChange`、`SyncPushOutcome`、`SyncPushReasonCode` 等静态类型，`packages/shared/src/schemas.ts` 导出对应运行时 schema，并在 server 路由和跨端同步边界收紧 UTC ISO 时间（严格 `YYYY-MM-DDTHH:mm:ss.sssZ`）、`#RRGGBB` 色值、整数排序、非空字符串、`QuickNote.text` 非空、`QuickNote.source` 只能是 `"user" | "agent"`、`QuickNote.sourceLabel` 最长 64 字符、`QuickNote.pinned` 只能是 boolean、`Task.title` 非空、`Task.completedAt` 严格 UTC 或 `null`、`Task.tags` 非空短字符串数组、重复规则字段、`Setting.value` 字符串值、`SyncLogEntry.synced` 的 `0 | 1` 数字状态、pull / force-push 请求形状等输入。同步 cursor 和计数字段（`baseSeq`、`sinceSeq`、`latestSeq`、`categoryCount`、`entryCount`、`quickNoteCount`）必须是有限非负整数或按契约允许 `null` / 缺省；后台洞察响应里的分页、计数、备份大小和备份 ID / 文件名也由 shared runtime schema 收紧。改这些 = 改公开 API。同步契约还承载本地优先诊断字段（如 `overriddenRecordIds`、`backupId`、`importedQuickNotes`、`importedTasks`），后台洞察契约承载最近同步问题和受保护备份元数据，三端展示/处理必须一起检查。
2. **时间用 ISO 字符串**：服务端 SQLite 的 `start_time` / `end_time` / `*_at` 都是字符串字段，比较直接靠字典序。Dexie 同样存字符串。新增记录页的时分滚轮只编辑本地时钟；`EntryPage.handleSave()` 会在未来校验前把“解析后结束时间比当前时间超前超过 12 小时”的范围按昨晚整体回退一天，用于凌晨补记昨晚，其余未来时间仍拦截。
...
9. **Tasks 是轻量待办域**：`Task` 存在于 Dexie `tasks` 与 SQLite `tasks`，通过 `syncLog(tableName="tasks")` 和 `sync_seq` 同步，冲突策略是 LWW，不进入手动冲突 UI，也不计入 `/api/sync/status` 的业务计数。`turn/turnAt` 是 opt-in 回合轴，表达等我处理、agent/脚本在跑、搁置；普通任务完成时写 `completedAt`，重复任务仍用 `lastDoneAt/completedCount`；`tags` 是自由语义标签，不驱动自动逻辑。完成仍复用 `done`，不另设完成回合态。`TodoPage` 输入用底部固定 composer 快速新增普通任务，默认提交进「今天」或用户设置的收件箱；顶部为 `AttentionQueue`（me/running/parked 三段注意力栈）+ `TagFilterBar`（OR 多选）。窄屏堆叠四区：**今天 → 已完成 → 已排期 → 收件箱**；宽屏（`min-width: 1024px`）用可拖拽 `ResizableSplit`，左 = 今天 + 已完成，右 = 已排期 + 收件箱。默认折叠态：今天常显（无开关）、已完成与收件箱默认展开、已排期默认折叠；折叠状态与双栏比例均存本地 `localStorage`，迁移仅作用于"未设偏好"的默认值。任务行是扁平双行基底：标题文本可选中复制，复选框独立完成；带子任务时左侧热区命中区展开内联子任务大纲（caret 仅为视觉指示，不再是独立按钮，点行经 `rowClickZone` 派发 expand），其余行体打开 `TaskDetailSheet`；零子任务的行不再渲染 caret 槽位，加子任务统一走详情抽屉的 `SubtaskEditor`。第二行只在需要时显示重复标记 `↻`、子任务进度 `m/n`、逾期日期（仅重复任务，文案 `逾期 M月D日`，取自 `currentDueDateString`）、被动排期小字（仅"已排期"区，由 `taskTimeLabel` 派生）、回合徽章与最多 3 个标签 chip。`listTasks` 把任务分桶为 `today` / `inbox` / `scheduled`（一次性未来排期 ∪ 未到期重复）/ `recurring`（去重 / AttentionQueue 用全集）/ `completed`（所有已完成 + 耗尽重复，按 `completedAt` 倒序，重复无 `completedAt` 排末）；落点算法仍在 `placement.ts`，`listTasks` 只在分桶层做渲染路由。今天↔InBox 换池仍走 `scheduleTask()` / `unscheduleTask()`，宽窄屏统一只走 **swipe + 详情抽屉**（行内不再有 ↑/↩/✕/纳/转/设定/时间 chip / hover「+」入口）：`TaskList` 按 `pool` 决定滑动菜单——`today` 允许「回收件箱 + 删除」trailing、`inbox`/`upcoming` 允许「排进今天」leading + 「删除」trailing、`completed` 只剩 destructive 「删除」trailing（`readOnly` 模式，不拖不换池），重复任务在所有 pool 下都因 `canSwap=false` 仅得删除滑动，`fullSwipe=false`。仅"今天"列允许 dnd-kit 手柄在同池内重排，已排期与收件箱不手动排序；重排通过 `persistTaskOrder()` 回填现有 `sortOrder` 槽位、更新 `updatedAt` 并写 `syncLog`，后续仍走普通同步。收件箱与已完成两区用 `DayGroupedList`（共享一卡 + hairline 分割线 + 行内日期标签，默认渲染最近 3 个日期组，多余进 `显示更多（N）`），收件箱按 `createdAt` 分天、已完成按 `completedAt` 分天（`completedAt` 缺失退化到 `updatedAt`）。详情抽屉内日期/重复编辑点击时间徽章打开 `RecurrencePresetSheet`；行内已无 popover 入口（早期宽屏行内日期 chip + `AnchoredPopover` + `RecurrencePopover` 三件套已下线，复杂规则统一走抽屉里的 `RecurrencePresetSheet` → `CustomRecurrencePage`）。`CustomRecurrencePage` 维护 `daily` / `weekly` / `monthly` 规则、间隔、可选本地时间、`due` / `completion` 基准、起始日期 `startAt` 和结束条件 `never` / `count` / `until`。月末规则用 `byMonthday: [-1]` 表达；已有多周几、多月号和混合月末规则在未显式改锚点/单位时保留，不静默塌成单日规则。任务池勾选翻转 `done`；重复任务勾选更新 `lastDoneAt` 并递增 `completedCount`，未终结时把 `subtasks[].done` 一并重置为 `false`（下一轮就绪），终结性完成（count 满 / until 过）保留勾选并进 `completed`；当前是否待做由 `isDueNow()` 按本地日序号计算。`count` 满或 `until` 完成最后一次后任务沉入完成区；`until` 已过但仍有逾期未完成发生时留在今天。点任务行或重复任务行从底部弹出 `TaskDetailSheet` 详情抽屉，头部显示完成复选框、重复/日期徽章、子任务计数和由子任务完成数派生的进度，并自动保存标题、日期/重复规则和子任务。子任务顺序即 `task.subtasks` 数组顺序；列表内联编辑器支持勾选、编辑和新增，详情抽屉的 `SubtaskEditor` 额外保留 dnd-kit 拖柄重排，二者经现有 `tasks` 同步域保存，不新增同步域。server 提供只读 `GET /api/tasks?kind=pool|recurring&done=0|1` 查询入口；授权 agent 回写状态走 `POST /api/agent/tasks/:id/status`，只允许封闭动作并通过 `sync_seq` 下发。
   - 代码入口：`packages/client/src/pages/TodoPage.tsx`、`packages/client/src/pages/todo/TaskRow.tsx`、`packages/client/src/pages/todo/TaskColumn.tsx`、`packages/client/src/pages/todo/TaskList.tsx`、`packages/client/src/pages/todo/SortableTaskRow.tsx`、`packages/client/src/pages/todo/TaskDetailSheet.tsx`、`packages/client/src/pages/todo/DayGroupedList.tsx`、`packages/client/src/pages/todo/AttentionQueue.tsx`、`packages/client/src/pages/todo/TagFilterBar.tsx`、`packages/client/src/pages/todo/RecurrencePresetSheet.tsx`、`packages/client/src/pages/todo/RecurrencePresetList.tsx`、`packages/client/src/pages/todo/ResizableSplit.tsx`、`packages/client/src/pages/todo/CustomRecurrencePage.tsx`、`packages/client/src/pages/todo/CollapsibleSection.tsx`、`packages/client/src/components/MonthCalendar.tsx`、`packages/client/src/components/Wheel.tsx`、`packages/client/src/components/ui/Sheet.tsx`、`packages/client/src/lib/useIsWideScreen.ts`、`packages/client/src/lib/tasks.ts`、`packages/client/src/lib/tasks/inboxGrouping.ts`、`packages/client/src/lib/tasks/taskTimeLabel.ts`、`packages/client/src/lib/tasks/workbenchPrefs.ts`、`packages/client/src/lib/tasks/taskRowZone.ts`、`packages/client/src/lib/tasks/taskSort.ts`、`packages/client/src/lib/tasks/turnTags.ts`、`packages/client/src/lib/tasks/recurrence.ts`、`packages/client/src/lib/tasks/recurrencePresets.ts`、`packages/server/src/routes/tasks.ts`、`packages/server/src/sync/domains.ts`
   - 相关测试：`packages/client/src/pages/TodoPage.test.tsx`、`packages/client/src/pages/todo/TaskRow.test.tsx`、`packages/client/src/pages/todo/TaskColumn.test.tsx`、`packages/client/src/pages/todo/TaskDetailSheet.test.tsx`、`packages/client/src/pages/todo/DayGroupedList.test.tsx`、`packages/client/src/pages/todo/AttentionQueue.test.tsx`、`packages/client/src/pages/todo/TagFilterBar.test.tsx`、`packages/client/src/pages/todo/RecurrencePresetSheet.test.tsx`、`packages/client/src/pages/todo/CustomRecurrencePage.test.tsx`、`packages/client/src/pages/todo/ResizableSplit.test.tsx`、`packages/client/src/pages/todo/TodoComposer.test.tsx`、`packages/client/src/pages/todo/SubtaskEditor.test.tsx`、`packages/client/src/components/MonthCalendar.test.tsx`、`packages/client/src/components/TimeRangeWheelPicker.test.ts`、`packages/client/src/components/ui/Sheet.test.tsx`、`packages/client/src/lib/useIsWideScreen.test.ts`、`packages/client/src/lib/tasks.test.ts`、`packages/client/src/lib/tasks/inboxGrouping.test.ts`、`packages/client/src/lib/tasks/taskTimeLabel.test.ts`、`packages/client/src/lib/tasks/workbenchPrefs.test.ts`、`packages/client/src/lib/tasks/taskRowZone.test.ts`、`packages/client/src/lib/tasks/taskSort.test.ts`、`packages/client/src/lib/tasks/turnTags.test.ts`、`packages/client/src/lib/tasks.recurrenceChoice.test.ts`、`packages/client/src/lib/tasks/recurrence.test.ts`、`packages/client/src/lib/tasks/recurrencePresets.test.ts`、`packages/client/src/lib/tasks/placement.test.ts`、`packages/server/src/routes/tasks.test.ts`、`packages/server/src/routes/sync.test.ts`
