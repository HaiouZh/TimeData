...
6. Router 注册时间轴、速记、待办、轨道、目标、时间/健康统计、设置及记录编辑路由；AppShell 按 `1024px` viewport 断点分流：窄屏 / APK 渲染底部纯图标导航并继续使用 `nav.visibleTabs.v1`，宽屏渲染左侧固定纯图标侧栏并使用 `nav.desktopSidebar.v1` 的排序 / 更多收纳配置；导航配置只保存 route / placement，不保存颜色。两套主导航按钮都必须有 `aria-label`，active 形态只用 `accent` / `surface` / `border` token。`/goals` 先进入目标页宽窄分流壳：宽屏默认全局星图只读总览，窄屏默认列表，并允许手动切换；`/tracks` 与 `/tracks/:id` 包在 `TracksShell` 布局路由里（宽屏甘特常驻的 master-detail，窄屏纯透传，见 [tracks](tracks.md) §8）。目标详情 `/goals/:id` 与轨道详情 `/tracks/:id` 在窄屏隐藏底部导航，宽屏仍保留桌面侧栏。MobileBottomNav 的更多菜单只在底栏可见时存在，底栏隐藏会同步收起弹层；设置子路由包含导航、轨道行动标签、统计布局、健康范围、服务端/数据/管理等入口，具体归属见各主题文档。
7. `SyncProvider` 在云同步开启且配置完整时维护 SSE 连接，并向模块级 `syncScheduler` 注册 executor；写入、SSE bump、回前台、隐藏前 flush、重连成功、60 秒兜底等触发统一经调度器合并防抖后驱动普通同步。调度器语义（防抖+max-wait、补跑、兜底、生命周期接线）见 [sync](sync.md) §1.6。
