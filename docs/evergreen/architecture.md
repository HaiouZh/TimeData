...
covers:
  ...
  - packages/client/src/components/ui/AnchoredPopover.tsx
  - packages/client/src/components/ui/popoverPosition.ts
...
关键点：`tasks` 是轻量待办域，不引用分类或时间记录。普通任务池条目用 `done` 表示完成，并用 `completedAt` 记录完成时间；重复任务用 `recurrence` + `startAt` + `lastDoneAt` + `completedCount` 计算当前是否到期和是否已耗尽。`turn/turnAt` 是可选回合轴，表达“等我 / 在跑 / 搁置”的当前状态和进入时间；`tags` 是自由标签数组，只供人/agent 表达语义，不驱动自动逻辑。完成仍以 `done=true` 为唯一真相。无终止条件的重复任务勾选后继续循环；带 `count` 或 `until` 的重复任务在做满/无后续发生后置为 `done=true` 并沉入完成区。待办首页列表展示复选框、根任务名、必要元信息和可展开子任务大纲；子任务可在列表内勾选、编辑和新增，也可在详情抽屉编辑，二者共用同一套草稿/提交语义。窄屏的日期/重复编辑仍下沉到详情抽屉；宽屏任务行提供日期 chip，使用锚定 popover 复用同一套预设门。重复设置走“徽章 → 预设门 → 自定义整页”：常用的每天、工作日、每周、每月和月末一击写入，复杂规则进入自定义页；已有多周几、多月号和 `byMonthday: [-1]` 月末规则不会因打开/完成而静默降级。未到期的重复任务只在“重复 / 提醒”区管理，非重复排期任务过期后回到收件箱，不堆进今天。服务端暴露只读 `GET /api/tasks` 供受控客户端查询；Web 端写入仍先落 Dexie，再经同步管线送到服务器；授权 agent 的状态回写走 `/api/agent/tasks/:id/status` 的封闭动作集合。
