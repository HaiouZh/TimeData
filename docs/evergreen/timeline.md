...
当客户端从后台恢复到前台时，应用会重新读取当前时间并刷新时间相关页面。时间轴的 `useNowMinute` 会用新的当前时间重新生成今天的末尾空档；新增记录页在没有 URL 时间参数时也会重新计算默认开始/结束时间，避免沿用上一次可见时的“现在”。
