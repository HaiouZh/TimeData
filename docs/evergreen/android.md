...
Android 壳入口是 `packages/mobile/android/app/src/main/java/app/timedata/mobile/MainActivity.java`。Activity 启动时关闭 decor 自动适配，并在根内容视图上应用 inset padding，让 Capacitor WebView 避开状态栏与刘海区域，避免 APK 在全面屏设备上把页面顶部绘制到通知栏下面。**顶部与左右照 `systemBars() | displayCutout()` 让位；底部恒为 0**——既不给手势条/导航栏让位（产品取向是内容延伸到横条之下、横条浮在其上，与 CSS 侧 `--safe-bottom: 0` 同源），也**不给键盘让位**（见下一段的 overlay 模型）。

**键盘让位走 overlay 模型（与 iOS resize:none 同一套）：壳完全不动，键盘盖在 WebView 上，输入条由网页层按插件高度用 transform 抬起**（见 [invariants 第 12 条](design-language/invariants.md)）。两个组成：① manifest 的 MainActivity 仍必须声明 `android:windowSoftInputMode="adjustResize"`——它禁掉系统 adjustPan（pan 平移整个窗口，`visualViewport` 无感、网页层按插件高度再抬一次 = 双倍避让，速记页输入条飞到屏幕顶），而 edge-to-edge（`setDecorFitsSystemWindows(false)`）下 `adjustResize` 本身不缩任何 view，正是 overlay 需要的「壳不动」；② MainActivity **不消费 `ime()` inset**。两条均由 `check-android-config.mjs` 棘轮住（含「不许消费 ime」的反向棘轮）。

**为什么壳侧让位（缩 WebView）被整体淘汰**——两条路都实测过：一次性 `setPadding` 时静态 insets 回调在键盘动画开始就带终态 inset，一次缩掉整个键盘高，WebView 底边与还在上升的键盘之间露出窗口背景（真机「先拉起一块输入法区域的白框、输入条等键盘就位才出现」）；改逐帧 `setPadding`（`WindowInsetsAnimationCompat.onProgress`）则被 `@capacitor/keyboard` 在 decorView 上注册的 `DISPATCH_MODE_STOP` 动画回调**拦停在根上**、子 view 的 onProgress 根本不触发（该插件靠这个回调发 keyboardWillShow/Hide 事件，且事件**不依赖壳让位**——`willShow` 在动画开始那一刻就带最终键盘高，这正是 overlay 模型的信源），即便触发，WebView 内容在渲染进程异步重画、缩的每一帧底部都先空后画。配套兜底：`AppTheme.NoActionBar` 的 `android:windowBackground` 与页面底色同源（`values/colors.xml` 的 `pageBackground` = `#0e1320`），任何缝隙不再白闪。
