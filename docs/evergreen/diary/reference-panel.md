...
<!-- 复核 2026-07-26（diary.md 拆子文档）：本文从 diary.md §5 原样外提，正文未改，仅章节号由 5.N 降为 N、跨文档引用改为相对链接。原 §5.1–§5.5 依次对应本文 §1–§5。 -->
...
本地三块走 `useLiveQuery`，正常路径只有 loading / empty 两态；它由 Dexie 托管订阅生命周期，**天然免掉手写「切日取消在途」的一整类竞态 bug**——阶段三在这类 bug 上栽过两次（见 [diary](../diary.md) §3.5）。本地 Dexie 读失败在实践中罕见，为它造一套 retry UI 是 YAGNI，所以三块**没有 retry 入口**，error/retry 只有走网络的「回看」块有。
...
> 这两条是阶段三用两轮返工换来的（[diary](../diary.md) §3.5 同源）。本页任何新的异步分支都照这两条办。
