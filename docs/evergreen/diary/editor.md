...
- **空列表项回车 = 逐级出层**：光标前无内容、行内光标后也无余文时不续号。**还有缩进可拿就先退一层**（`removableIndentLen`，与 Shift+Tab 同一份判据），编号交给整块重排按新层级算；**退到顶层再按一次才清空该行**。三级项要连按三次才清行，每次退一层。
  - 原语义是"任何深度的空项都直接清行（含缩进一并删）"，在嵌套清单里的实际表现是：二级空项一回车就吐出一行连缩进都没有的纯空行，而空行既不是列表项也没有缩进，紧接着按 Tab 想救回来时 `applyIndent` 的候选行过滤直接放行、焦点跳去下一个可聚焦元素（宽屏下正是分栏拖拽把手）。用户看到的是"回车吐出没序号的行、Tab 还会乱跳"。2026-07-27 改判，与 Notion / Typora / Obsidian 一致。
  - **顶层空项清行的语义一个字没变**（边界表 C09/C39/C41 原样保留），它是这条边界的另一半——顶层若也改成"再退一层"就没有出口了。
  - 出层后下方更深的项按 `expectedNumbers` 的单调栈重算层级（边界表 C55），出层行所在层级的后续项顺延（C42）——都是整段拉直的既定语义，不是出层额外引入的副作用。
- **两条回车分支共用一个块重排 helper**（`rebuildBlockAround`）：续号拆行与空项出层的唯一区别是"当前行被替换成哪几行"，其余（整块重排 → `trimEditSpan` 前后缀裁剪 → 光标公式 → 映射回 `value` 坐标系）逐字相同。分成两份写过一次，结果是出层分支漏抄了光标公式里"编号位数变化可能发生在光标上方"那半条。
...
marker 的四个位置都认全角（`listModel.ts` 的 `WS_CLASS` / `DIGIT_CLASS` / `DOT_CLASS`）：

| 位置 | 除半角外还认 |
|---|---|
| 缩进 | 全角空格 `U+3000`、不间断空格 `U+00A0` |
| 编号 | 全角数字 `０-９` |
| 分隔符 | 中文句号 `。`、全角句点 `．` |
| gap | 全角空格、不间断空格 |

**为什么放宽**：中文输入法下这四个位置极易混进全角，而原来只要混进一个，整行就不匹配 `ITEM_RE`，回车返回 `null` 交还浏览器 → 吐出一行没有序号的空行；紧接着按 Tab 也认不出那行，焦点跳去下一个可聚焦元素（宽屏正是分栏拖拽把手）。用户看到的是「回车吐空行、Tab 乱跳」，而全角字符肉眼与半角难分，自己排查不出来。

**输出一律规范成半角**（`renumberBlock` → `renderItem`）：缩进按**等宽**替换（`U+3000` → 两个空格，`U+00A0` → 一个空格），gap 收成半角空白，分隔符恒输出 `.`，编号经 `toHalfWidthDigits` 转半角。只放宽识别、原样写回是不够的——全角 marker 在 CommonMark / Obsidian 里根本不构成列表（行首的 `U+3000` 甚至不算空白，整行退化成普通段落），那会变成「我们这边认、渲染那边不认」的两套行为。规范化只发生在**本次真被改写的块**上，不去动文件里别处的行。

- **等宽替换是硬要求**：`visualCol(normalizeIndent(s)) === visualCol(s)` 必须恒成立（全角空格在 `visualCol` 里按 2 列算），否则规范化会顺手改掉嵌套层级。
- **`Number("１")` 是 `NaN`**：任何把编号当数字用的地方都要先过 `toHalfWidthDigits`，否则 `String(Number(numText) + 1)` 会把字符串 `"NaN"` 写进用户的日记（单项块路径下它就是最终值，无人兜底）。
- **`removableIndentLen` 按视觉列宽出层**，不是按字符数：一个全角空格已占 2 列，按字符数算会一次拿掉 8 列、出层直接跳两级。
- **已知代价（显式接受）**：中文正式文档里「１．这是一段」这类全角编号是常见的装饰性写法，现在会被当成列表项——光标落在它上面回车，整块会被拉直并规范成半角。
- 放宽只动 `ITEM_RE` 与附属行的行首空白口径，**没有绕过保护位**：围栏 / front-matter 内的全角行照样放行。
