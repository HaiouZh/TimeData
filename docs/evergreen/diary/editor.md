...
**识别口径钉死为 Markdown 标准（产品裁决，2026-07-27）**：`ITEM_RE` 只认半角数字 + `.` + 至少一个半角空白（空格/Tab）。全角数字、中文句号/全角句点、全角空格缩进或 gap、编号后无空格——这些"差一点就是列表"的写法**一律当普通文本**，回车放行原生换行，编辑器一个字节不改也不提示。两个方向都试过并被否决：①识别放宽 + 写回规范成半角（曾以 `62b38f54` 合入，同日 revert）；②提示条 / 一键修复。理由：编辑器要纯粹、符合标准，提示补丁会越积越多还阻碍正常编辑。将来再遇到"用户写了 `1.二级` / `１. 二级` 回车没续号"的反馈，那是符合预期的行为，不要再来一轮放宽救援。
