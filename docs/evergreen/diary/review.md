...
年份范围偏好默认 5，最小 1、最大 10；非法值回默认，写入时 clamp。它的调节入口只在 A 模式下渲染，切到 B/C 模式入口消失但偏好值保留。模式偏好只接受 `A/B/C`，B 模式布局偏好只接受 `grid/list`，坏值都回默认。三项偏好存 localStorage，不进 Dexie `settings` 同步表。
