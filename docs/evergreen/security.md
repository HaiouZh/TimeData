...
`resetLocalDataToDefaults`（「清空本地数据」）会在清空各表的同时一并清除这两个 `…ComposerDraft` 草稿 key，未发出的速记正文不会残留在本机。
