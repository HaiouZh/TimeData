...
- Android 系统返回键/边缘返回通过 `packages/mobile` 的 `@capacitor/app` 原生插件监听，并交给前端 `androidBackNavigation` 处理：根路径退出 App；设置二级页（含 `/settings/more` 更多功能）回 `/settings`（数据备份历史回 `/settings/data`、分类详情回分类列表）；轨道/目标详情分别回 `/tracks`、`/goals`；新增/编辑记录优先走 history back，兜底回时间轴；日记页同样优先 history back，兜底回速记页。
