...
pnpm test              # 递归执行各 package 测试，并运行根目录 scripts/*.test.mjs；根脚本串行执行 workspace，避免本地全量测试资源争用导致 Vitest hook 超时
...
根目录 `scripts/*.test.mjs` 使用 Node 内置 test runner，通过 `pnpm test:scripts` 单独运行，也会被 `pnpm test` 串起来，覆盖 docs 检查脚本等不属于 workspace package 的工具。
