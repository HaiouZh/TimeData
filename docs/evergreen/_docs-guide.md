...
architecture.md（地图）         第 1 层：只索引「主题文档」，不下探到具体文件
  └─ <主题>.md（主题文档）       第 2 层：拥有一个域 + 索引自己的子文档，目标 < 15k 字符
       └─ <主题>/<子>.md（子文档） 第 3 层：主题膨胀后沿某条轴外提的子文档，由主题文档索引
```
...
**下面三段两条轴共用**，只有准入判据分横 / 纵（§3.1 / §3.2）。

**外提动作**：把内容移到 `docs/evergreen/<主题>/<子主题>.md`（子目录，见 §1.1），**横切**要迁走对应 `covers:`、**纵切**不迁（母文档 `covers` 不变），在子文档顶部回链主题文档，在主题文档的“子文档索引”小节登记，重写链接与内部引用，再**全仓扫一遍入向引用**（下表第四类）。然后 `--write-size-baseline` 重写基线（新增子文档要落基线；横切还要更新母文档迁走后的 covers 数，纵切母文档 covers 数不变）。architecture 登记簿**不**新增子文档条目——它只认主题文档。
