...
- **长时间后台回来画面冻住、几秒后整页重载**（能看到旧界面但什么都点不动，重载后仍在原页、无启动画面）：iOS 在内存压力下回收了 WKWebView 的 Web Content Process，屏幕上留的是系统快照，`CAPBridgeViewController` 检测到进程终止后重载同一 URL，代价是前端整套冷启动。**与调度器死锁的鉴别见 [ios/scheduler-resilience](ios/scheduler-resilience.md) §4.5**——死锁时 JS 还活着，现场是「弹层点得开、tab 点不动」的半瘫；「滑不动」两族都会出现，不具区分力。归因数据在 `sync_logs` 的 `cold_start` 上报里（`detail.cause` 为 `external` 即本条）。
