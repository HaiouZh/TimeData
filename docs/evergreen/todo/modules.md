...
| `lib/taskNesting.ts` | 用户视角的收纳/升根复合动作：`nestTaskUnderParent`（降级为 child，同事务清 `sessionId` + 所有 `goal.members`，见 [todo](../todo.md#todo-s2-2)）、`promoteTaskToHand`（子任务升根并直接站到手头，串行 `promoteToRoot` + `grabTaskToHand`，见 [at-hand](at-hand.md)）、`promoteTaskToProject`（组内子任务升根并**留在本组**，串行 `promoteToRoot(…, "inbox", …)` + `assignTaskToProject`——先落 inbox 只是升根的中转姿态，紧接着的入组会把它从收件箱排他掉，见 [project-zone](../project-zone.md#project-zone-nesting)）。落在本文件而非 `tasks.ts`/`goals.ts`：`goals.ts` 已单向 import `tasks.ts`，反向引用会成环，复合动作只能置于两者上层 |
...
交互图标统一经 Phosphor `Icon` 包装（规则见 [design-language](../design-language.md#design-language-s4)），按钮语义由文本与 `aria-label`（如 `删除标签 ${tag}`）承载。
