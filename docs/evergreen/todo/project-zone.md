...
- **落点反馈**：排他打开后「回到 inbox 池」不再等于「出现在收件箱」——项目成员会落进项目区里一个默认折叠的组，而组 header 的「还剩 N / 共 M」本来就把它算在内、数字纹丝不动，全屏零反馈。故凡是让成员回落 inbox 池的路径，动作后都要展开它的归属组：行尾/左滑「回收件箱」、拖进 `pool:inbox`、移出手头（无排期）、子任务升根、详情抽屉「不重复」清时间、已完成区取消勾选。判据一律是**落点真的是 inbox 池**（用 `placementForTask`），排到未来的成员回的是已排期区、本来就看得见，不展开。查归属分两级：先查 `projectChipIndex`（渲染期闭包，覆盖"动作前就是未完成根成员"的情形），未命中再 `findActiveProjectGoalIdForTask` 读一次库——**子任务与已完成成员不在任何客户端投影里**，只有查库找得到。
