...
车道判定吃的是 `delta.x`——相对**起手点**的位移；而坞画在**绝对位置**——来源区块左缘（`anchorLeftPx`），宽 176px（`TODO_DOCK_WIDTH_PX`）。两者是不同的坐标系。
...
因此判定含一项几何：指针落在坞矩形（含右缘 `TODO_DOCK_HOLD_BUFFER_PX` = 16px 缓冲，防贴边抖动）内时，`holdDock` 短路释放。页面在 `handleDragMove` 里用 `起手 clientX + delta.x` 换算指针视口 x 后传入。
...
| 态 | 条件 | 视觉 | 命中 |
|---|---|---|---|
| `hidden` | 未拖拽，或 `targets` 为空（手头源等） | 全透明 | 车道非 dock，不参与 |
