...
CLI 的 `task-running` / `task-handback` / `task-park` / `task-done` / `task-tag` 调 `/api/agent/tasks/:id/status`，只做任务回合状态和 tags 回写：`running` 表示外部执行中，`handback` 写 `turn="me"` 并可用 `--note` 追加结果备注，`park` 写 `turn="parked"`，`done` 写 `done=true` 并清空回合状态，`tag` 写自由 `tags`。该 API 接受 master `AUTH_TOKEN` 或窄域 `AGENT_TOKEN`，服务端仍通过 `applyChange()` + `sync_seq` + `notifySyncChange()` 下发到前台客户端。
