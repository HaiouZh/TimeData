...
| `timedata task-done --id ID` | 是 | 标记任务完成（重复任务衍生已完成记录并推进模板） | `POST /api/agent/tasks/:id/status` |
...
- `task-done`：root task 完成语义走 server `completeTask`——非重复任务就地完成、重复任务衍生一条已完成快照并推进模板（count/until 终结时模板就地转化）；child task 若被明确指定 id，则只轻量更新自身 `done/completedAt`，不调用 root-only 的 `completeTask`。详见 [todo](todo.md) §3.1。
