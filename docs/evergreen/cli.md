...
| `timedata task-done --id ID` | 是 | 标记任务完成（重复任务代理完成「最新那一发」occurrence） | `POST /api/agent/tasks/:id/status` |
...
- `INVALID_DATE`、`INVALID_TIME_RANGE`、`CATEGORY_NOT_FOUND`、`CATEGORY_AMBIGUOUS`、`TIME_OVERLAP`
- `INVALID_JSON`：请求体不是合法 JSON；`INVALID_BODY`：是合法 JSON 但未过 POST body schema（两者都是 400，别当同一个）
