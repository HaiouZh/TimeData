...
| `unseen_record_deletion_rejected` | `needs_arbitration` | 服务端拦下「会隐式删除本设备从未见过的记录」的写入。客户端三件事一起做：标 `synced=2` 移出上传队列、把完整 change 存进纯本地 `arbitrations` 表、放入同步问题列表。见下方「隐式删除守卫」。 |
