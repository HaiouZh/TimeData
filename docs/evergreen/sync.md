...
当前十四个运行时域：`categories`（manual，钩子承载层级校验与级联删除）、`time_entries`（manual，钩子承载未来时间拒绝、重叠覆盖）、`settings`（lww，零钩子，`countsInStatus=false`，承载睡眠分类、打点分类、底部导航可见入口等 UI 偏好）、`quick_notes`（lww，零钩子）、`tasks`（lww，零钩子，`countsInStatus=false`）、`tracks`（lww，`countsInStatus=false`）、`track_steps`（lww，`countsInStatus=false`）、`goals`（lww，`countsInStatus=false`，目标层）、`health_charts`（lww，健康统计页视图块配置），以及 5 个健康数据域 `health_heart_rate` / `health_hrv` / `health_sleep` / `health_stress` / `runs`（均 lww、零钩子、`countsInStatus=false`，走通用 LWW 路径）。
...
**新增一个纯 LWW 域的全部成本**：shared 登记一行 + server 写 `lww` 映射和 `readRecord` + 客户端 Dexie 表与 pull 应用分支。校验、排序、写入、记账、seq 补差、墓碑、SSE 实时下发全部白捡。任务轨道和目标层都是这个模式：`tracks.upsertPriority=70/deletePriority=71`，`track_steps.upsertPriority=71/deletePriority=70`，`goals.upsertPriority=72/deletePriority=72`。验收证明见 `packages/server/src/sync/fake-domain.e2e.test.ts`、`tracks-domain.e2e.test.ts` 与 `goals-domain.e2e.test.ts`。
...
- `syncForcePushToServer()`：确认后把本地核心同步表覆盖到服务器；当前只包含 `categories`、`time_entries`、`settings`、`quick_notes`、`tasks`，不包含健康原始数据、`health_charts`、任务轨道或 `goals`。
...
1. `orderPushChanges`（登记簿优先级驱动）：**categories upsert（组内父子拓扑排序）→ time_entries → settings → quick_notes → tasks → 健康域 → health_charts → tracks → track_steps → goals**，delete 也按各域 `deletePriority` 进入同一排序。保证 entry 引用的分类先到位、轨道步骤父先建子先删。
...
force-push 语义不变：shared schema 校验 → 跨记录业务校验（分类、记录、速记、任务 ID 不重复；分类不自引用不三级；父分类与记录分类存在；时间段不互相重叠）→ 先 `createServerBackup('sync_force_push')` → 单事务清空 `sync_tombstones` / `sync_seq` / `categories` / `time_entries` / `quick_notes` / `tasks`，有 `settings` 载荷时也清空 `settings`，然后导入这些核心表 → 刷新 commit hash → 写审计日志 → `notifySyncChange`。健康原始数据、`health_charts`、`tracks`、`track_steps` 与 `goals` 当前不在 force-push 请求、校验和导入范围内；但 tasks payload 的 `goalId` 会写入 `tasks.goal_id`。确认 token 为进程内 Map、TTL 5 分钟、一次性消费；多实例限制同前。
...
冲突只发生在 manual 域（categories / time_entries）。lww 域（settings / quick_notes / tasks / tracks / track_steps / goals 等）后写赢，自动解决。
