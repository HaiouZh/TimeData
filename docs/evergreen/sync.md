...
当前十个域：`categories`（manual，钩子承载层级校验与级联删除）、`time_entries`（manual，钩子承载未来时间拒绝、重叠覆盖）、`settings`（lww，零钩子，`countsInStatus=false`）、`quick_notes`（lww，零钩子）、`tasks`（lww，零钩子，`countsInStatus=false`），以及 5 个健康域 `health_heart_rate` / `health_hrv` / `health_sleep` / `health_stress` / `runs`（均 lww、零钩子、`countsInStatus=false`，走通用 LWW 路径）。
...
- **通用 LWW**（settings、quick_notes、tasks 及未来的零钩子域）：delete = 真删除 + tombstone upsert；upsert = 删 tombstone + `INSERT ... ON CONFLICT DO UPDATE`（列来自域的 `toRow()`，主键与 `created_at` 只在插入时写）。
