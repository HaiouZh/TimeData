...
6. 服务器 200 返回后按 `SyncPushOutcome.reasonCode` 分类处理本地 syncLog：`applied`、`client_bug` 和 `stale_rejected` 类标已同步；`user_actionable` / `conflict` / `unknown` 保留。HTTP 409 表示整批原子拒绝，任何 `accepted/applied` outcome 都只代表“通过校验”、不能确认日志；客户端剔除问题项后立即重试合法子批，只有重试 200 后才确认。`stale_change_rejected` 会进入 `pushIssues`，但客户端放弃本地主张，随后回声 pull 落地服务器权威版本。
...
3. **任意一条 invalid 就整体拒绝**：返回 409 + 全部 outcomes，不写业务表、`sync_seq`、tombstone 或 SSE；accepted outcome 的文案只表述 passed validation。
...
| reasonCode | 分类 | 客户端处理 |
|---|---|---|
| `applied` | `applied` | 200 响应中表示已应用并确认日志；409 原子拒绝中仅表示通过校验，必须等合法子批重试成功后确认。 |
