...
**登记簿是封闭的**：加域必须改代码、过测试。静态类型 `SyncChange`（`types.ts` 手工判别联合）与运行时 schema（登记簿生成）必须同步修改。当前已知风险：运行时登记簿已包含 `health_charts`，但手工 `SyncChange` 联合还缺该分支；补齐时按“改公开 API”处理，跨 client/server/cli 一起检查。
...
- `getSyncHealth()`：contentHash 深度体检 + 建议；本地 content hash 与服务端 commit hash 都会受 `tasks` 变化影响，但 `/api/sync/status` 的公开计数字段仍只返回分类、时间记录和速记数量。
- `syncForcePushToServer()`：确认后把本地核心同步表覆盖到服务器；当前只包含 `categories`、`time_entries`、`settings`、`quick_notes`、`tasks`，不包含健康原始数据或 `health_charts`。
...
1. `orderPushChanges`（登记簿优先级驱动）：**categories upsert（组内父子拓扑排序）→ time_entries → settings → quick_notes → tasks → 健康域 → categories delete**。保证 entry 引用的分类先到位、entry delete 先于 category delete。
...
3. **域钩子**：
   ...
   - `settings` / `quick_notes` / `tasks`：无钩子，通用校验即全部。
...
- **通用 LWW**（settings、quick_notes、tasks、health_charts、健康数据域及未来的零钩子域）：delete = 真删除 + tombstone upsert；upsert = 删 tombstone + `INSERT ... ON CONFLICT DO UPDATE`（列来自域的 `toRow()`，主键与 `created_at` 只在插入时写）。
...
force-push 语义不变：shared schema 校验 → 跨记录业务校验（分类、记录、速记、任务 ID 不重复；分类不自引用不三级；父分类与记录分类存在；时间段不互相重叠）→ 先 `createServerBackup('sync_force_push')` → 单事务清空 `sync_tombstones` / `sync_seq` / `categories` / `time_entries` / `quick_notes` / `tasks`，有 `settings` 载荷时也清空 `settings`，然后导入这些核心表 → 刷新 commit hash → 写审计日志 → `notifySyncChange`。健康原始数据与 `health_charts` 当前不在 force-push 请求、校验和导入范围内。确认 token 为进程内 Map、TTL 5 分钟、一次性消费；多实例限制同前。
...
冲突只发生在 manual 域（categories / time_entries）。lww 域（settings / quick_notes / tasks）后写赢，自动解决。
...
待办域的重复规则、turn/tags、排序、完成状态和子任务重置都只是 `tasks/update` 载荷内容：客户端 helper 通过既有 Dexie transaction 写 `tasks` + 本地 `syncLog`，授权 agent 回写构造 `tasks/update` 后走 `applyChange()` + `sync_seq` + SSE 通知。它们不新增同步域，也不改变 `tasks` 仍是通用 LWW 域的服务端契约；字段语义与展示桶见 [todo](todo.md)。
...
- [ ] **加新域**：shared `syncDomains.ts` 登记 → server `domains.ts` 写 `lww` 映射 + `readRecord`（复杂域加钩子）→ `types.ts` 扩展 `SyncChange` 判别联合 → 客户端 Dexie 表 + engine pull 分支 → force-push / backup 是否纳入当前格式契约 → 参照 `fake-domain.e2e.test.ts` 写全链路测试 → 更新本文档第 0 节。`health_charts` 当前是需要补齐静态 `SyncChange` 的已知例外，不要以它为新增域模板。
