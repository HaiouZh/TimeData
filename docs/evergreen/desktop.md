...
**主窗口侧仍无焦点兜底**：`show_main_window` 与 capture 侧同形状，`toggleMain` 与 navigate 都复用它。主窗口抢不到焦点的后果轻得多——窗口显示出来就看得见，不构成「看着能打字却打进别处」的半开状态，故按已知界限留着。

**但 `show` / `unminimize` 的失败会记一行 stderr**（`log_window_op`，两个窗口同款）：它们失败意味着窗口根本没显示，「按了热键窗口没出来」时手上得有线索。**`set_focus` 刻意不记**——前台锁让它降级后照样返回 `Ok`，记了只会制造「日志干净所以没问题」的错觉。不弹提示是同一条判断：`show()` 失败几乎无从恢复，弹窗只是把无力感传给用户。**这行日志的界限**：打包后的 GUI 进程没有控制台，只在从终端启动时看得见；要生产可查得引入日志插件（未做）。
