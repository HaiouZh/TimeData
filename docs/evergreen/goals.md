...
`/goals` 在宽屏默认进入全局星图只读总览，窄屏默认保留列表，两端都提供“星图 / 列表”切换。全局星图只展示 `status:"active"` 的 Goal；archived Goal 仍在列表的归档区，不进入画布。
...
C1 全局画布只读：不拖拽、不连边、不写 pin。双击恒星进入 `/goals/:id` 聚焦编辑器；聚焦编辑器工具栏提供“返回目标星图”回到 `/goals`。全局星图顶部 HUD 显示 canvas-level rollup（全局完成百分比、本周推进、在动 Goal），它只读现有 Goal/Task/Track/Step 快照。
...
| `client/src/pages/goals/GoalGalaxyCanvas.tsx` | 只读全局星图 React Flow 画布、HUD、双击恒星导航 |
