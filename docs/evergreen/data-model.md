...
| 时间记录 / `TimeEntry` | `time_entries` | `timeEntries` | [timeline](timeline.md) 与本文 §2 |
| 速记 / `QuickNote` | `quick_notes` | `quickNotes` | [quick-notes](quick-notes.md) §Schema |
| 待办 / `Task`、`Recurrence` | `tasks` | `tasks` | [todo](todo.md) §Schema |
| 同步设置 / `Setting` | `settings` | `settings` | 本文 §3；具体 key 见对应域 |
| 客户端同步日志 / `SyncLogEntry` | 无 | `syncLog` | 本文 §4 |
...
- `startTime` / `endTime` / `createdAt` / `updatedAt` 都是 UTC ISO。
- `endTime > startTime`；服务端同步校验会拒绝 `invalid_time_range`。
- `categoryId` 必须存在，且引用分类不能 archived；同一批 push 新增的分类也算存在。
- 与同 `id` 之外的记录不能重叠，区间按半开 `[start, end)` 判断。
- 时间轴展示、跨夜、空档、相邻合并和本地写入事务见 [timeline](timeline.md)。
...
`tableName` 来自 `packages/shared/src/syncDomains.ts` 的封闭登记簿，当前包括 `categories`、`time_entries`、`settings`、`quick_notes`、`tasks`、`health_charts` 与健康原始数据域。Dexie 使用 `[tableName+synced]` 复合索引。同步实体写入与对应 `syncLog` 追写必须在同一个 transaction 内完成。

## 5. SyncChange / SyncPushOutcome

`SyncChange` 是按 table/action 区分的判别联合；运行时 schema 由 `packages/shared/src/syncDomains.ts` 的登记簿生成，静态类型在 `packages/shared/src/types.ts` 手工维护。新增同步域必须同时改共享登记簿、服务端登记簿、类型、测试和文档。
...
全量推送兜底复用完整数据集请求体。服务端在导入前校验分类/记录/速记/任务/设置/健康图表的 ID 唯一性、父分类存在、记录引用分类存在、时间范围合法、记录之间不重叠等规则。
...
- `SyncForcePushRequest` / `SyncForcePushResponse`：完整数据、服务器备份 ID、导入计数与最新 seq。
...
服务端 `sync_seq` 是同步 cursor 的权威来源。每次 `applyChange()` 成功写入同步实体后追加一行；CLI `log` 成功创建时间记录后也追加 `time_entries/create` 序列；force-push 全量覆盖服务器时清空并重建序列。
...
JSON 字符串列：`recurrence`、`subtasks`、`tags`、`health_charts.config`。布尔列通常以 0/1 存储。
...
版本历史：v1 初始；v2 `settings`；v3 `quickNotes`；v4 健康表；v5 `tasks`；v6 `tasks.scheduledAt`；v7 `healthCharts`。

## 12. SQLite schema 迁移边界
