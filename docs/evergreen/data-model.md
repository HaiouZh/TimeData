...
> 当前已知风险：运行时登记簿包含 `health_charts`，服务端和客户端也能同步该域；但 `packages/shared/src/types.ts` 的手工 `SyncChange` 联合尚未列出 `health_charts` 分支。改这里等于改公开 API，需单独跨 client/server/cli 检查后补齐。
...
全量推送兜底当前只覆盖核心同步表：`categories`、`timeEntries`、可选 `settings`、`quickNotes`、`tasks`。服务端在导入前校验分类/记录/速记/任务的 ID 唯一性、父分类存在、记录引用分类存在、时间范围合法、记录之间不重叠等规则；健康原始数据与 `health_charts` 当前不在 force-push 契约内。
...
JSON 字符串列：`recurrence`、`tags`、`health_charts.config`。布尔列通常以 0/1 存储。`tasks.parent_id` 是普通 TEXT（可空），不是 JSON。
...
版本历史：v1 初始；v2 `settings`；v3 `quickNotes`；v4 健康表；v5 `tasks`；v6 `tasks.scheduledAt`；v7 `healthCharts`；v8 `tasks.parentId`（子任务=独立 Task，纯 schema 升级无 upgrade 函数）。
