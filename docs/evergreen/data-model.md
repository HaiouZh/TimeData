...
- `sortOrder` 是客户端展示排序字段。TodoPage 按 `sortOrder` 展示，落点（今天 / InBox / 即将到来 / 完成）由 `lib/tasks/placement.ts` 按 `scheduledAt` 或重复规则的 `startAt` 决定；完成区在数据层按 `completedAt` 倒序兜底排序。UI 的「今天 / InBox / Repeat」分组只是这些落点的展示折叠，不改变数据模型，换池仍走 `scheduleTask` / `unscheduleTask`。今天、即将到来和重复列表允许同池拖拽重排，收件箱不手动排序；`lib/tasks/taskSort.ts` 先按该池现有 `sortOrder` 取槽位，再按新 ID 顺序回填变化项，`persistTaskOrder()` 在一个 Dexie transaction 内更新 `sortOrder/updatedAt` 并为每个变化项写 `syncLog(tableName="tasks", action="update")`。详情抽屉和 composer 的“仅某天”预设通过 `applyRecurrenceChoice()` / `addTask({ scheduledAt })` 一次写成普通排期任务，避免重复写 `syncLog`。
