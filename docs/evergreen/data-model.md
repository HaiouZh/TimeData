...
JSON 字符串列：`recurrence`、`tasks.tags`、`tracks.refs`、`track_steps.tags`、`track_steps.refs`、`goals.members`、`goals.prerequisites`、`health_charts.config`。布尔列通常以 0/1 存储（`tasks.done`、`tasks.skipped`、`categories.is_archived` 等）。`tasks.parent_id` 和 `track_steps.track_id` 都是普通 TEXT 列；`track_steps.track_id` 不建 SQL 外键。旧 `tasks.goal_id` / `tracks.goal_id` 目标归属列已退役，启动迁移会幂等删除。
