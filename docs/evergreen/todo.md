...
4. **四分区是读时视图**：`today` / `inbox` / `scheduled` / `completed`，另有全量去重桶 `recurring` 供标签来源去重。P3 后 `today` 只读 pending occurrence（`ruleId!==null && !skipped && !done`），重复模板不再投影到今天，退到 `scheduled` 规则管理区；`scheduled` = 一次性未来排期 + 重复模板，按下一发生日升序，行内显示重复摘要与下一发生日；`completed` 收纳普通完成任务、重复 occurrence 完成快照与终结模板，按 `completedAt` 倒序、**无日期过滤**。改 `recurrence` 或 `startAt` 视为重锚：清 `lastDoneAt`/`completedCount`，同事务级联删旧活跃 occurrence 及其 children，再按新规则即时尝试物化；规则/起始日未变则保留进度。
...
7. **子任务 = 独立可拖 `Task`（`parentId` 一层）**：见 §2.2。child 勾选不联动父 `done`/`completedAt`（父进度 `m/n` 由 `InlineChildren` 实时聚合，不回写父行）。pending occurrence 物化时克隆模板当前 children，保留当时 `done`/`completedAt`/`tags` 快照；Today 展开的是这一发自己的 children，不回退读取模板 children。**重复 root 完成时处理 children（历史快照）**：`completeTask` 读 reset 前 children，同一次返回 `occurrenceChildren`（克隆为指向 occurrence 的独立 child，保留 `done`/`completedAt`/`tags`，并清 `recurrence`）与 `templateChildren`（同批 child reset 成 `done=false`/`completedAt=null`）；客户端 `toggleTaskDone` 与 agent `done=true` 共用这套，事务内写 occurrence + occurrence/template children + 模板。历史 occurrence 的 children 在「已完成」内只读显示。
...
| `lib/tasks.ts` | 核心 CRUD + `listTasks`（P3：today 读 pending occurrence、重复模板退 scheduled、skipped 排除、`recurring` 空桶兼容）/`putTask`；child helper `createChildTask`/`promoteToRoot`/`moveTaskToParent`/`deleteTaskCascade`；`toggleTaskDone` 对 child 走非重复路径、对 pending occurrence 完成后即时物化下一发、对 root 取 reset 前 children 委托 `completeTask` 并事务写 occurrence + occurrence/template children + 模板；`bumpTaskWeight` 累加 `weight` 并写 syncLog；`markOccurrenceSkipped` 删·跳这一发（skipped 留痕）并即时物化下一发；`runMaterialization` 遍历 rule 物化当前 occurrence + occurrence children（in-flight 合并 + 事务内二次检查）；`updateTask` 重锚时同事务级联删活跃 pending occurrence 并即时尝试物化新 occurrence；`applyRecurrenceChoice` none/scheduled 同事务清孤儿 |
