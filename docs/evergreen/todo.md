...
| `lib/tasks.ts` | 核心 CRUD + `listTasks`（P3：today 读 pending occurrence、模板退 scheduled、skipped 排除、`recurring` 空桶兼容）/`putTask`；child helper `createChildTask`/`promoteToRoot`/`moveTaskToParent`/`deleteTaskCascade`；`toggleTaskDone` 对 child 走非重复路径、对 root 取 reset-前 children 委托 `completeTask`，同事务写 occurrence + occurrence/template children + 模板；`bumpTaskWeight` 累加 `weight` 并写 syncLog；`markOccurrenceSkipped` 删·跳这一发（skipped 留痕）；`runMaterialization` 遍历 rule 物化当前 occurrence（in-flight 合并 + 事务内二次检查）；`updateTask` 重锚时同事务删活跃 pending occurrence；`applyRecurrenceChoice` none/scheduled 同事务清孤儿 |
