...
| `busy` | 正在检查更新… | 否 |
| `idle` | 当前版本：X | 是（手动检查） |
| `idle` + 有错 | 当前版本：X · 〈失败原因原文，超 60 字符截断加省略号〉 | 是（手动检查） |
| `disabled` | 开发构建不检查更新 | 否 |

「上次失败」仍并列显示当前版本：只显示失败会让用户不知道自己停在哪个版本上，而那恰是失败时最该知道的事。

**显示的是原因原文，不是一句「上次检查失败」**：Rust 侧存进 `last_error` 的三类原因（网络失败 / 验签失败 / 安装失败）处置完全不同（§3.4），折叠成一句等于把这个区分丢掉——用户连开数周也判断不出是网络抖动（会自愈）还是 CI 配错（要人工动作）。副标题是单行，故超 60 字符截断。
...
`check-desktop-config.mjs` 中与本主题相关的六条（配错时构建、测试、其余断言全绿，装机后更新静默失效）：
...
倒数第二条是接插件时补上的缺口：原正则只认 `@tauri-apps/api`，插件包能穿过全部门禁。

最后一条守的是 Rust 与 TypeScript 之间那几个**裸字符串**——四个 `PHASE_*` 常量的值，与三个 `updater_*` 命令名。两端没有共享类型，typecheck 管不到字符串字面量：Rust 侧改一个字，两边测试全绿、CI 全绿，而前端把未知 phase 一律当 idle 静默降级（检查期间那行显示「当前版本：X」直到完成，没有任何报错），命令名对不上则 invoke 直接 reject。

**闸的扫描面覆盖 `api.ts` 与 `SettingsPage.tsx` 两个文件**，因为 phase 的显式判定散在两处：前者决定副标题文案，后者决定点击是装还是查、「可更新」角标显不显示、行禁不禁用。只扫其中一个就只堵了一半——改名时同步改了副标题却漏改设置页，闸照绿而安装链路静默断裂。这与 §2 事件名那类「只认 `app.emit(` 就会被 `emit_to` 绕开」是同一个形状：**闸的扫描面漏一处，等于那一处没有闸。**

`idle` 不出现在任何显式判定里（它是 if 链末尾的 fallback），所以闸断言「Rust 侧恰好一个值落在 TS 显式集合之外」。加第五个 phase 且前端不打算单独显示它时，这条会误报——届时要么给它补一句显式分支，要么放宽这条断言。方向是保守的：宁可错杀，不会漏报。
