...
- **导出**：`exportQuickNotes` 产 JSON（独立备份格式 `timedata.quick-notes.backup`，`quick-notes/schema.ts`，`timeFormat:"utc"`，与主 `timedata.backup` 是两套契约）或 Markdown（同分钟/间隔 ≤5min `MARKDOWN_TIME_GAP_MS` 不重复 `## HH:mm` 时间标题）。按日期范围导出的入口在 设置 → 数据（`SettingsDataPage`）；速记页内只剩多选导出（选中若干条 → Markdown / JSON），页面不再提供按天导出。下载经 `fileDownload.ts`（Blob / 原生 Filesystem+Share）。
...
10. **速记页 UI 要点**：聊天式连续时间线，最新窗口（`QUICK_NOTE_PAGE_SIZE=50`）向上懒加载；搜索 200ms debounce、空格分词 AND、只读扫描 Dexie，入口在 composer 空草稿左按钮；搜索结果按本地日倒序分组，初始最多渲染 100 条并用「加载更多」继续展开；结果卡片本体不可**点**（避免误触被拽离搜索流）但可长按 / 右键 / 键盘唤起操作菜单——与主线气泡同一套 `noteInteractionProps` 接线，菜单四项：复制 / 置顶 / 删除留在搜索态原地完成（结果列表是 live 查询，删了置顶了自动刷新），「编辑」= 先走定位那条路跳到时间线再进入编辑（composer 只在时间线才有），**不提供「选择」**（进多选会自动退出搜索，`QuickNoteActionMenu` 的 `onSelect` 为可选项、不传即不渲染）；跳时间线走卡片角上的「定位到时间线」小按钮：非置顶结果保留搜索词、锚定目标所在时间线窗口、滚动到主线卡片并 1.5 秒内侧高亮，置顶结果展开置顶区并跳到日期；点日期分隔条选日期后等窗口就位再把目标日分隔条滚到视口顶（当天无速记则回列表顶，那里就是目标日之后最近的内容）；退出搜索/点「最新」会把 URL `?date=` 归位到今天；**「回到最新」两种过场**（`quick-notes/scrollToLatest.ts`）：已在最新窗口就平滑滚到底，离底超过两屏先瞬移到离底两屏处再滚——Telegram Web-A `fastSmoothScroll` 同款，动画时长不随距离拖长，滚动期间浮标按真实位置到底才消失（不预先 `setAtBottom(true)`，否则闪三下）；不在最新窗口只能换数据硬切，新窗口落定那一刻列表内层 150ms 淡入（Web Animations 直接打在 DOM 上，不加 state）；系统减弱动效两处都瞬切。吸底 layout effect 的依赖是 `displayItems` **引用**而不是条数：`atLatest` 在新数据到达前就翻 true、那一跑贴的是旧内容的底，两窗条数相等（7 天 50 条 → 7 天 50 条）时按条数就不再跑，列表停在旧底、浮标不消失（曾实测离底 3587px）；淡入也据此只认「点击之后内容真的换过」；**常态不渲染顶栏**（那一行只在多选 / 搜索时出现，那两个态本来就要整行放工具条），「历史」徽标与置顶钉子改走右上角浮层——浮层容器 `pointer-events-none`、只有按钮与面板收回点击，否则会盖住第一屏气泡的长按与点选；置顶区从这颗浮动钉子展开（多选态改由顶栏那颗展开，两处共用同一份 `renderPinnedPanel()`），主线过滤 pinned；composer 空草稿右按钮打点、有草稿左右分别存待办/记录、编辑中左右分别取消/保存；agent 气泡用动作蓝 soft 形态 + sourceLabel 标题；长按/右键开复制/编辑/置顶/选择/删除菜单（气泡菜单与多选态导出菜单均可 Escape 关闭），选择态支持批量复制/导出/删除；多选态置顶区保持可见可勾选（header 有钉子开关，计数在含置顶时显示「含置顶 M」，保证批量删除目标集不含不可见项）；工具栏「全选」只吃已加载的非置顶速记（与「清理跳过置顶」同一保护口径，反选保留置顶勾选），日期分隔条 `position: sticky` 粘顶跟随，静止 0.3 秒后粘住那条 150ms 淡出隐身、再滚动 150ms 淡入（`STUCK_HIDE_DELAY_MS` 与 `.quick-note-date-divider` 的过渡；**隐身是静止时的常态，不是「滚过一次之后才有」**——扫描既从滚动停手排、也从列表就位排：打开页面、退出搜索 / 多选、关掉日历这几条路上一次 scroll 都没有，只认滚动就会让药丸常驻在顶上不消失；程序化日期跳转那条**刻意不排**，刚跳过去要看得见落点是哪一天，等用户真正动手滚才恢复跟随（`skipNextScrollScanRef` 与 `pendingJumpRef` 两道一起让开）；0.3s 是产品按手感定的，TG Android 是 0.5s、iOS 停手即淡，淡出时长照 Android 的 `floatingDateAnimation`；多选态与日历打开期间不扫描：否则「选中这天」点不到、月历失去锚点）；**浮动（粘顶）中的日期条点了不开日历**——Telegram 双端同款，浮动条与原位条在本仓是同一个 sticky 元素，只能在点击那一刻按几何分辨（`isStuckCandidate`，区间与停手扫描共用；捕获阶段只拦药丸触发钮，月历 Sheet 走 portal 冒泡经过时不受影响），列表原位那颗才是跳转入口；药丸本体几何与颜色只在 `index.css` 的排版角色语义类 `.quick-note-date-pill`（字号 12px = 正文 15px × 0.8、500、行高 14px、内距 3×8px 撑到 20px 高、无边框、`backdrop` 混 55% 透明底、`ink` 字；比例取 Telegram iOS 的 13pt / 17pt，内距与底色照 Android 的 `ChatActionCell`），主线 `DateField bare` 与搜索态纯 div 两处都只挂这一个类，有用例锁两处同款；**主线**每条日期条自身即跳转入口（`DateField` + `hideIcon`），常态没有顶栏，日期条上除跳转外不挂任何按天操作，多选态下额外挂一个「选中这天」toggle——**搜索态的日期条刻意是纯展示、不可点、里面不含任何 button**（误触会把人从搜索流里拽走，有专门用例锁死）；**速记页没有按天导出 / 清理**：这套操作在 设置 → 数据 里按日期范围提供（`SettingsDataPage`，JSON / Markdown 导出 + 按范围删除），页内不再重复一份。页内导出只剩多选那条路径——选中若干条后导出 Markdown / JSON、复制、删除。

  这里曾有一套 `viewingDate`「跟随眼前那天」机制，服务于页内按天导出 / 清理的菜单：停手扫描把粘顶那条的日期写进 state，菜单读它当目标日。它要求**每条会挪动视图的链路都显式归位**——退出搜索、程序化落点、日历跳转、搜索结果定位、回到最新——漏一条就是「清理今天」静默变成「清理 6月X日」，而导出侧没有二次确认。菜单退役后 state 与它的五处归位调用一并删除；停手扫描只剩「给粘顶那条打隐身类」这一职，`cancelPendingStuckScan()` 守的也只剩「过期定时器别给错的日期条打隐身类」。长文本按渲染高度折叠（`COLLAPSED_MAX_PX=168` + ResizeObserver）。页面壳、气泡、菜单、Markdown chrome 和搜索高亮全部消费 [design-language](design-language.md) 的 `page/surface/border/ink/accent` token；业务时间、日期和数量使用 `td-time` / `td-num`，当前是 Times New Roman / Tinos + tabular nums；交互图标统一经 Phosphor `Icon`，不使用散装字符图标。
11. **sticky 日期条有三条跨文件红线**（不知道就会踩）：① **每天必须各自成一个包裹 div**——`position: sticky` 的约束框是父级内容盒，同一父级下的 sticky 兄弟互不推挤；拍平渲染时滚过去的每条日期条都恒停在偏移位、层层相叠，`findStuckDivider` 会把窗口里最早那天误判成粘顶那条，`.stuck` 于是打在被压在底下看不见的那条上，真正粘住的那条反而一直显形、与列表里同一条重影。分组由纯函数 `quick-notes/dayGroups.ts:groupDisplayItemsByDay` 产出，包裹 div 不得带 `overflow`/`contain`/`transform`/`filter`（任一都会掐断内部 sticky 或改写包含块）。有结构闸用例断言两天的日期条不共享父节点。② **`QuickNotesPage.tsx:STICKY_TOP_PX` 必须等于 JSX 上 `sticky top-2` 的实际像素**（0.5rem = 8px）——它是 `findStuckDivider` 判定区间的上界，三处独立字面量（常量 + 主线/搜索两处 className）改一处不同步既不报错也不 typecheck 失败，由导出常量派生类名的绊线用例守。③ **滚动容器 `<section>` 自己不许带 padding-top，顶部留白放列表内层**——Chrome 与 WebKit 都按滚动容器的**内容盒**算 sticky 的约束矩形，容器带 `py-5` 时 `top-2` 的日期条实际粘在 8 + 20 = 28px 处（真浏览器实测，两个引擎同值），而 ② 里那个常量仍是 8：区间永远框不到粘顶那条，`.stuck` 一次都打不上、药丸常驻，`blockPickerWhileFloating` 共用同一区间、「浮动药丸不开日历」也一并失效；jsdom 不排版，用例里 mock 的几何绕得开这 20px，2026-09-16 那次「列表就位也排扫描」的修复正是因此在线上没有效果。有结构闸用例断言滚动容器的 class 与 inline style 里没有任何 padding-top。
