...
6. **横向溢出从组件源头收口**：全站 `<main>` 负责纵向滚动，交互组件若会产生临时横向位移（如 Todo 拖拽 / swipe 行），应在组件行容器或本主题全局规则里裁掉横向溢出，避免把页面撑出横向滚动面；纵向拖拽让位可单独放开。**推论：swipe 行内的装饰必须画在内侧**——`ring-*` 与任何向外画的 `box-shadow` 会被祖先 `.todo-swipe-row { overflow:hidden }` 整圈裁掉，真机不可见，而 jsdom / happy-dom 不算裁剪，只断言 className 的单测照样全绿（"已归目标任务绿外圈"就这么 ship 成过隐形功能）。用绝对定位子元素（`pointer-events-none`，避开圆角与拖拽命中区）或 `ring-inset`，并靠真机 / 截图验收——单测在这件事上给不出结论。
...
12. **底部避让量单一来源**：速记页（QuickNotesPage）与待办页（TodoPage）喂进上一条 `calc()` 组成式的 px 项，由 `lib/bottomInset.ts` 的 `composeBottomInset({ barHeightPx, navOffsetPx, keyboardHeightPx })` = `Math.ceil(三者之和)` 单一合成，是两页共用的唯一入口。各页私有的"此刻底部站着谁"（QuickNotes 的 selectionMode/searchOpen 分支、Todo 的多选/滚动收起分支）仍留在各页自己算，只把结果当 `barHeightPx` 喂给合成函数——合成函数本身不判断"底部站着谁"。`keyboardHeightPx` 由 `hooks/useKeyboardHeight.ts` 的 `useKeyboardHeight()` 给出，语义是**「键盘还挡着页面底部多少」**（= JS 还需要额外让开的量），**不是键盘自身的高度**：壳自己已经让过位时它就是 0。口径是**TG 单源：native 只信插件事件，web 只信实测**（Telegram 双端同构——安卓等真实 Resize、iOS 等 WillChangeFrame，源码采掘与对抗验证见 `.dispatch/20260822-tg-reference`、`.dispatch/20260822-kbd-statemachine` 两份报告）：

    1. **native**：`keyboardWillShow` 带真实高度（安卓在 IME inset 动画 onStart 即发、已除 density 为 CSS px；iOS 走 UIKit 通知），`keyboardWillHide` 归零——事件到达即起步，输入条的 `.td-kbd-motion` 过渡与 IME 动画同向滑动。壳缩量（`innerHeight` 相对「键盘不在场基线」的落差）逐次 resize 如实扣除，**不留跨次记忆**（记忆值在壳行为变化时是「飞半空 / 抬不够」的温床）；基线只在键盘不在场（插件高度为 0）时校准。`visualViewport` 实测**不参与 native**：多源互相校正正是「飞半空 / 收起悬空」整族竞态的来源，真机读数（2026-08-22，安卓）也证明壳不动时 vv gap 与插件高度同值、无需第二源。
    2. **web / PWA**：无插件桥接，`visualViewport` 与 `innerHeight` 的差值（`innerHeight - viewport.height - viewport.offsetTop`，> 80px 阈值防地址栏抖动误报）是唯一信源。

    **不做预测抬升**：focusin 不按记忆值预抬——Telegram 双端都等真实键盘信号（kbd_height 类缓存只用于表情面板估高，不驱动输入条位移），预抬的「先动一段再被事件校正」两段运动正是真机「唤起卡顿」的主体，预测值也是多源竞态的一极。focusout 到非可编辑目标把高度清零（换输入框不闪落），是 `willHide` 丢失（进程重建 / 外接键盘）时的自愈出口。**指按即聚焦**（`lib/fastFocus.ts` 的 `focusOnPointerDown`，挂在三条 composer 输入框上）只在非 iOS 生效：iOS 的 WKWebView 里 pointerdown 中 `preventDefault()` + `focus()` 会把触摸序列判成 cancelled，touchend 时 resign firstResponder——键盘闪现即收回、无法唤出（穷举验证见 `.dispatch/20260822-ios-flash`）；鼠标与已聚焦元素不拦（保留拖选与 caret 点位语义）。

    **两个 hook 共享一份模块级 store**（`useSyncExternalStore` 订阅，第一个消费方挂载时起监听、最后一个卸载时停并复位）。**不是风格选择**：各消费方各挂一套监听时，「键盘不在场时的 innerHeight」这条基线各算各的，**键盘已弹起之后才挂载**的消费方会把被壳压缩过的高度当基线——它的 shellShrink 恒为 0、报出的遮挡量比先挂载的那个多一整段（两条底部固定条一个停在键盘上沿、另一个飞在半空）；`useKeyboardVisible` 侧同理，晚挂载者初值 false 而键盘正弹着，要等下一次事件才对得上，而那次事件可能永不到来。切页面、展开带输入条的面板都会造出这种晚挂载。store 里 height 与 visible 两侧的内部状态**各留一份**（两条基线的校准条件不同：visible 按「缩量 ≤ 阈值」、height 按「插件报的高度 ≤ 0」），共享的只是那套监听与分发。

    **「还挡着多少」与「在不在场」是两个信号，不可混用**：同文件另导出 `useKeyboardVisible()`——native 平台两路信源：插件事件（inset 动画驱动、不依赖壳让位，安卓 willShow 在动画开始即发）+ 壳缩实测兜底（壳若缩 webview 时与 IME 动画同步、只升不降）；web 实测兜底。**同样不做 focusin 预测性在场**（消费方先动一段再被校正 = 上蹿下跳）。focusout 到非可编辑目标即离场（换输入框不闪落；也是插件缺席 / 外接键盘时收回信号的唯一自愈出口），并与 `keyboardWillHide` 同款竖起缩量压制，防壳恢复途中的中间帧把在场顶回。桌面浏览器（platform=web）不挂 focus 监听，敲字不收底栏。「键盘弹起时收起底栏」「composer 的隐藏是不是键盘引起的」这类**在场判断**必须用它：拿 `keyboardHeightPx > 0` 判在场，在安卓壳层让位后恒假——底栏不收、缩短的视口里输入条与键盘之间杵一条 tab 行，composer 还会被 navHidden 联动误藏。消费点：Todo 页的收底栏 effect / `navOffsetPx` 守卫 / `composerHiddenByScroll` 守卫，速记页的 `inputInteractionActive`。避让量合成（`composeBottomInset` 的 `keyboardHeightPx`）仍走 `useKeyboardHeight`，两者各管各的。插件的 `addListener` 在插件缺席时**返回 rejected promise 而非同步抛**，两个 hook 都在返回处同步 `.catch` 接住（否则 AppShell 挂 `KeyboardAvoidanceBridge` 后，凡 mock 平台为 native 又没 mock 插件的测试整文件炸 unhandled rejection）。

    **为什么不能直接信 `Keyboard.resize: none`**：该配置**只有 iOS 读**——Android 端 `KeyboardPlugin.java` 只读 `resizeOnFullScreen`，`setResizeMode` 是 `unimplemented()`；而 iOS 侧 `none` 也只拦住插件自己 resize，拦不住 WebKit 因聚焦输入框而挪视口。故避让口径不建立在「壳一定不动」之上，而是插件高度减实扣壳缩量。**两平台统一 overlay 模型**：壳完全不动（iOS = `resize:none`；Android = manifest `adjustResize` 禁 pan + `MainActivity` 不消费 `ime()` inset，含反向棘轮，机制与被淘汰的壳层让位两条老路见 [android](../android.md#android-s2)；2026-08-22 真机读数证实 `innerHeight` 全程恒定），键盘盖在 WebView 上，插件路径（`rawKeyboardPx - 壳缩量 - 视口平移量`，正常路径后两项恒 0）即为两端主路径；壳缩量逻辑保留作防漂移兜底（分屏、外接、壳行为变化）。**引擎侧同时用 `index.html` 的 `interactive-widget=overlays-content` 叫 Chromium 撒手**：没有它，Blink 会为露出聚焦框平移视觉视口（reveal-pan），所有 fixed 元素（输入条、tab 栏）视觉整体上移一个键盘高、与 JS 抬升叠成「输入框飞到屏幕中间」；不认该参数的 WebView 由 `vv.offsetTop` 几何扣除兜底（锚点校正，不是第二个键盘尺寸信源）。iOS WebKit 不识别此参数、忽略无害。
    ...
    **键盘运动（位移全走动画，Telegram 式）**：底部固定条（Todo composer / Todo 多选栏 / 速记 composer）**统一由 `components/KeyboardDock.tsx` 驻坞定位**——TG 结构对齐：全应用只有一份输入条定位实现（fixed + 安全区 + 抬升合成 + navOffset 守卫 + 运动曲线 + 壳缩抹平 + zIndex），页面只提供内容，禁止另写第二份定位（此前两页各持手工副本，时序各自漂移，见 docs/notes/keyboard-pipeline-saga.md）；键盘引起的底栏实体收起同样只有 `useKeyboardNavCollapse()` 一份（记账式，滚动引起的隐藏不碰），**且只在 web / PWA 生效**：native 走 overlay 模型，底栏本来就在键盘背后，收它只产生一段高度 / 布局动画、恰好挤在输入条位移的首帧（重列表就卡一下），故 native 一律不收——`navOffsetPx` 的 `!keyboardVisible` 守卫仍让输入条压在键盘上沿、不留缝；iOS Safari 会滚文档、底栏会露到键盘上方，web 保留收起。载体分工——`bottom` 只装安全区、恒定不动，动态抬升（navOffset / 键盘高的合成值）走 `transform: translateY(-抬升量)` 吃 `.td-kbd-motion` 的过渡（`index.css` 只声明 `transition-property: transform`；**时长与曲线由 `useKeyboardMotion()` 内联**：浮层 override > 实测的系统时长 > 平台默认，单一来源 `lib/keyboard/keyboardMotionPrefs.ts`——在场用 `showMs`、不在场用 `hideMs`；平台默认 iOS 250/250 + UIKit 键盘曲线近似 `cubic-bezier(0.38, 0.7, 0.125, 1)`，Android 285/285 + `InsetsController.SYNC_IME_INTERPOLATOR` `cubic-bezier(0.2, 0, 0, 1)`，web 250/200 + Telegram DEFAULT_INTERPOLATOR `cubic-bezier(0.25, 0.1, 0.25, 1)`；合成器线程、无重排）。**实测时长**：store 同时听插件 `keyboardDidShow / keyboardDidHide`（动画结束发：Android `onEnd`、iOS `UIKeyboardDid*Notification`），`did − will` 落在 [80, 700] ms 才采信、写 `keyboardMotion` key，取最近一次不做均值（换输入法要立刻跟上）；新的 will 覆盖未闭合的起点（iOS 候选条加高会再发一次 willShow）。只学时长、不预测高度，两平台键盘弹起/收起的位移因此是与 IME 同段的滑动而非瞬跳（overlay 模型下抬升值就是全部位移来源）。**固定条不挂 `backdrop-blur`**：模糊与 transform 位移动画同帧是移动端掉帧经典组合，TG 输入条同为实心底。壳缩/恢复 webview 的单帧跳变（overlay 下正常路径不发生，防分屏/壳行为变化兜底）由 `lib/keyboardMotion.ts` 的 `useShellResizeGlide` 抹平：`innerHeight` 每变化一个键盘量级（≥80px，与实测阈值同源同值）就给固定条叠一段「从跳变量滑回 0」的**附加**动画（`composite: "add"`，不打断基础 transform 的抬升/滚动隐藏；即 Telegram Android 用 ValueAnimator 平移 parent 的 Web 等价物）——首帧视觉位置不变、随后按同一份 `useKeyboardMotion()` 的 `showMs` / 曲线滑到新位置（KeyboardDock 把同一参数传给它，两种运动衔接不断速）。web 平台不补偿（桌面拖窗不该看到内容滑动），`el.animate` 缺席（旧 WebView / jsdom）时静默退回瞬跳。

    **键盘动画期间零重排（native）**：内容留白与 sticky 偏移喂 `useKeyboardHeightSettled()`——did 事件（或「时长 + 50ms」超时）后才跟上即时值，web 与即时值同步；输入条位移（KeyboardDock）、贴 composer 的浮层与 Bridge 的差值滚动仍用即时值。**iOS 锁外层滚动**：Bridge 挂载时 `Keyboard.setScroll({ isDisabled: true })`（只 iOS），见 [ios §3.3](../ios.md#ios-s3-3)。

    **探针常驻**（`components/KeyboardProbe.tsx` 接线、`lib/keyboard/kbdProbe.ts` 纯逻辑）：设置 → 数据 → 「高级 · 诊断」开关打开后，每次弹起→收起攒成一条 `kbd_session` 走 `pendingReports` 搭同步车进服务端 `sync_logs`（`action` 自由串、`detail` ≤ 1000 字符，超长截事件尾并标 `trunc`），字段：平台 / OS / 构建 / 路由 / 聚焦在驻坞条内还是页面内 / 各事件相对聚焦的毫秒与当时 `scrollY`、`vv.offsetTop` / 首帧延迟（事件到下一 rAF）/ 量到的系统时长 / 本次输入条用的时长曲线 / 底栏收没收。每次打开预算 20 条、扣完自动关（`pendingReports` 只 30 格，别挤掉 `open_session`）；关着时不挂任何监听。store 只 `subscribeKeyboardEvents` 广播事件、不认识探针；focusin 只广播不改状态。读数浮层（`KeyboardDebugOverlay`）是探针的附属显示（探针开 + 浮层开），带 show± / hide± / 曲线 / 重置调参按钮，写 override 后 KeyboardDock 即时跟上。报告：`node scripts/kbd-report.mjs`（只读生产库）。

    闸：`indexCssTokens.test.ts`（CSS 规则）、`KeyboardAvoidanceBridge.test.tsx`（变量写入、聚焦滚动、焦点释放、iOS setScroll）、`App.keyboardInset.test.tsx`（AppShell 接线）、`keyboardMotion.test.tsx`（跳变补偿）、`KeyboardDock.test.tsx`（同参、平台曲线、native 不收底栏）、`useKeyboardHeight.test.tsx`（did 学时长、settled、探针总线）、`keyboardMotionPrefs.test.ts` / `kbdProbe.test.ts`（纯逻辑）、`KeyboardProbe.test.tsx` / `KeyboardDebugOverlay.test.tsx`（接线与调参）、`TodoPage.keyboard.test.tsx` / `QuickNotesPage.keyboard.test.tsx` / `TodoComposer.test.tsx`（载体分工：bottom 恒装安全区、抬升在 transform；留白喂 settled）。
