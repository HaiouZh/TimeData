...
2. 构造 `BackupDocumentV2`（含 `timeFormat: "utc"`），`exportedAt` 用当前时间，`appVersion` 从 `import.meta.env.VITE_APP_VERSION` 读，缺省 `0.1.0`。
...
| `INVALID_FORMAT` | `format` 不是 v1 或 v2 |
| `UNSUPPORTED_FORMAT` | `format` 是 v1（旧版本，不再支持导入） |
...
2. **同 ID 分类保留当前名称**：导入外部旧备份时，如果当前库里已有同 ID 分类，导入流程保留当前分类名称，避免旧备份把分类名改回去；记录仍通过 `categoryId` 正常关联。
...
当前是 `v2`（`timedata.backup.v2`）。**改格式的规则**：

1. 升 `BACKUP_FORMAT_V2` → `BACKUP_FORMAT_V3`，加新常量。
2. v1 不再支持导入（`UNSUPPORTED_FORMAT`）；v2 保留校验和读取。
3. 导出永远写最新版本。
4. `validateBackup` 识别 v1（拒绝并给出明确提示）和当前版本，不识别的其他格式返回 `INVALID_FORMAT`。

详见 [`adr/0003-backup-format-versioning.md`](../adr/0003-backup-format-versioning.md)。

## 8. 改这块代码前的清单

- [ ] 跑 `packages/client/src/backup/*.test.ts`：导出、校验、恢复都有测试。
- [ ] 改 schema：升版本号，**不要原地改 v1**。
