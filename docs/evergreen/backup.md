...
`timeFormat` 恢复前必须存在且值只能是 `"utc"`。`timeEntries` 的 `startTime` / `endTime` 必须是带毫秒和 `Z` 的 UTC ISO 字符串且 `endTime > startTime`。任务时间字段同样 UTC `.sssZ` 或 `null`，重复规则满足 shared `RecurrenceSchema`；终止式重复的 `count` / `until` 随 `recurrence` JSON 保存，`completedCount` 记录已完成次数，`turn/turnAt` 记录当前回合状态与进入时间，缺省会归一为 `completedCount=0`、`turn=null`、`turnAt=null`。分类树只支持两级结构。
...
- `createAutoBackup()` 把当前 `categories` + `timeEntries` + `tasks` 整盘存进 Dexie `autoBackups` 表；settings 和 quick notes 不进入自动滚动备份，分别走同步管线和独立速记备份恢复。如果最新一份自动备份的数据签名与当前数据相同，会跳过写入，避免连续同步把可恢复快照挤掉；签名使用按 `id` 稳定排序后的完整分类/记录/任务内容 JSON，确保数量和最新更新时间不变但字段内容不同的情况也会生成新快照，任务的 `completedCount`、`turn/turnAt`、`recurrence.count`、`recurrence.until` 都参与签名；最多保留 **7 份**（FIFO，按 `createdAt` 倒序）。
