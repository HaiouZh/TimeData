...
- `createAutoBackup()` 把当前 `categories` + `timeEntries` 整盘存进 Dexie `autoBackups` 表；如果最新一份自动备份的数据签名与当前数据相同，会跳过写入，避免连续同步把可恢复快照挤掉；签名使用按 `id` 稳定排序后的完整分类/记录内容 JSON，确保数量和最新更新时间不变但字段内容不同的情况也会生成新快照；最多保留 **7 份**（FIFO，按 `createdAt` 倒序）。
