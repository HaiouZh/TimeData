...
恢复前必跑。`validateBackup` 用 shared 包的 `CategorySchema`、`TimeEntrySchema`、`TaskSchema` 与 `UtcIsoStringSchema` 严格校验分类、记录、任务和时间字段。正常通过 TimeData 客户端导出的备份文件不会受影响；手工编辑过的备份如果时间字段不带毫秒、不带 `Z` 或带时区偏移，会被拒绝，建议改成 `2026-05-19T03:00:00.000Z` 这种 `.sssZ` 格式后重试。
...
- `createAutoBackup()` 把当前 `categories` + `timeEntries` + `tasks` 整盘存进 Dexie `autoBackups` 表；settings 和 quick notes 不进入自动滚动备份，分别走同步管线和独立速记备份恢复。如果最新一份自动备份的数据签名与当前数据相同，会跳过写入，避免连续同步把可恢复快照挤掉；签名使用按 `id` 稳定排序后的完整分类/记录/任务内容 JSON，确保数量和最新更新时间不变但字段内容不同的情况也会生成新快照；最多保留 **7 份**（FIFO，按 `createdAt` 倒序）。
