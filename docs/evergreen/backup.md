...
`timeFormat` 恢复前必须存在且值只能是 `"utc"`。`timeEntries` 的 `startTime` / `endTime` 必须是带毫秒和 `Z` 的 UTC ISO 字符串且 `endTime > startTime`。任务时间字段同样 UTC `.sssZ` 或 `null`，重复规则满足 shared `RecurrenceSchema`；终止式重复的 `count` / `until` 随 `recurrence` JSON 保存，`completedCount` 记录已完成次数，缺省会归一为 0。分类树只支持两级结构。
...
恢复前必跑。`validateBackup` 用 shared 包的 `CategorySchema`、`TimeEntrySchema`、`TaskSchema` 与 `UtcIsoStringSchema` 严格校验分类、记录、任务和时间字段。正常通过 TimeData 客户端导出的备份文件不会受影响；手工编辑过的备份如果时间字段不带毫秒、不带 `Z` 或带时区偏移，会被拒绝，建议改成 `2026-05-19T03:00:00.000Z` 这种 `.sssZ` 格式后重试。任务记录经 `TaskSchema` 归一化，旧记录缺少 `completedCount` 时按 0 恢复；`recurrence.count` 与 `recurrence.until` 同时存在会被拒绝。
...
- `createAutoBackup()` 把当前 `categories` + `timeEntries` + `tasks` 整盘存进 Dexie `autoBackups` 表；settings 和 quick notes 不进入自动滚动备份，分别走同步管线和独立速记备份恢复。如果最新一份自动备份的数据签名与当前数据相同，会跳过写入，避免连续同步把可恢复快照挤掉；签名使用按 `id` 稳定排序后的完整分类/记录/任务内容 JSON，确保数量和最新更新时间不变但字段内容不同的情况也会生成新快照，任务的 `completedCount`、`recurrence.count`、`recurrence.until` 都参与签名；最多保留 **7 份**（FIFO，按 `createdAt` 倒序）。
