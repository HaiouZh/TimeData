...
- `createAutoBackup()` 把当前 `categories` + `timeEntries` + `tasks` 整盘存进 Dexie `autoBackups` 表；settings、quick notes、tracks/track_steps 和健康域不进入自动滚动备份，分别走同步管线和完整 Backup JSON 恢复。如果最新一份自动备份的数据签名与当前数据相同，会跳过写入，避免连续同步把可恢复快照挤掉；签名使用按 `id` 稳定排序后的完整分类/记录/任务内容 JSON，确保数量和最新更新时间不变但字段内容不同的情况也会生成新快照，任务的 `completedCount`、`weight`、`completedAt`、`tags`、`recurrence.count`、`recurrence.until` 都参与签名；最多保留 **7 份**（FIFO，按 `createdAt` 倒序）。
