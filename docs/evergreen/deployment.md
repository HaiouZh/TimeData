...
- `ci.yml`：push / PR 的基础 CI，安装依赖后依次运行 `pnpm lint`、`pnpm -r typecheck`、`pnpm -r --parallel test`、evergreen 文档一致性检查和 `pnpm build`，不发布产物。文档检查按发起人区分：dependabot 触发的 PR 走 `pnpm check:docs`（warn，不阻塞），其余走 `pnpm check:docs:strict`——dependabot 只动 `package.json` 的依赖版本号，会被 `development.md` 的 `covers` 命中但本身不需要同步文档，降级 warn 避免每次依赖升级都误报。
