...
> 部署一台服务器要填哪些变量、CORS 怎么放行、归属地库怎么装，见子文档 [deployment/configuration](deployment/configuration.md)。
...
环境变量表与默认值、CORS 白名单与壳 origin 内置放行、自定义请求头白名单与预检缓存、GeoLite2 与中国段表的安装与降级形态，见子文档 [deployment/configuration](deployment/configuration.md)。
