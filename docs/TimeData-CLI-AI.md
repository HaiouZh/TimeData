...
# 归档轨道（status=concluded 顺手闭合开口步）
curl -X PATCH "$BASE/tracks/track-refactor-auth" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"status":"concluded"}'
```

> 看板信号机制：Web 端扫描 active 轨道最近一条带已配置看板信号的步骤，按标签聚合计数并支持 OR 筛选。默认信号为 `agent在做`（人派活给 agent）与 `待我处理`（agent 完成或需要人接手）。agent 完成任务或需要人处理时 append 一个新步骤，打 `待我处理` 或用户当前配置中的等价看板信号；无标签步骤和普通检索标签不会清掉当前看板信号。
