...
个人时间记录 PWA。按时间线记录一天中的活动，也可以像聊天一样快速写速记；支持两级分类、时间空档识别、统计图表、本地离线存储，以及和自托管服务器同步。
...
- 定期备份 `data/` 目录
- 升级前先备份数据库
- 在网页设置页定期导出完整 Backup JSON；速记可单独导出 `timedata.quick-notes.backup` JSON 或 Markdown
