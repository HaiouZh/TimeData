...
- 默认允许并提倡；用户许可 > 工具约束。挑可并行、低冲突、可独立验收的活（入口梳理、影响面、覆盖缺口、交叉验证、review），琐碎活自己干。
...
- 合完清理 worktree（Windows）：按 `git worktree prune` → `git branch -D <branch>` → `rm -rf <path>` 收尾；`git worktree remove` 在 Windows 常报错，别纠结，直接走这套。
