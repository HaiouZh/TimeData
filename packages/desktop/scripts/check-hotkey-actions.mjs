...
/// 热键动作名一致性校验（纯函数，不碰文件系统）。
///
/// **抽成纯函数只为立一道闸**：原本整份脚本是「顶层直接执行 + readFileSync 读真实路径」，
/// 消费分支检查那段（见下面 `if (!consumesAction(...))`）被架空时没有任何东西会发现——
/// 实测变异确认过：把那段改成 `if (false && …)`，跑 test 仍 exit 0、照常打印成功行。
/// 于是校验逻辑抽出来接收源码字符串，本脚本只负责读文件后调用它；自测
/// （check-hotkey-actions.test.mjs）用夹具喂假源码验证它真的会红。
export function checkHotkeyActions({ actionIdSource, apiSource, settingsSource, shellSource, consumerSources, consumerPaths }) {
  ...
  const union = UNION.exec(apiSource);
  if (!union) {
  ...
  const options = OPTIONS.exec(settingsSource);
  if (!options) {
  ...
  for (const [, variant, windowConst, isNone] of arms) {
    ...
    if (isNone) {
      ...
      // 而那正是「热键按下去没反应」的成因。
      for (const [label, source] of Object.entries(consumerSources)) {
    ...
    const source = consumerSources[label];
    if (!source) {
      throw new Error(
    ...
    if (!consumesAction(source, id)) {
      throw new Error(
        `[hotkey-actions] 动作 "${id}" 投给窗口 "${label}"，但 ${consumerPaths[label]} 里找不到 ` +
          `event.action === "${id}" 的消费分支。热键会注册成功、按下去零反应，且没有任何测试会红。`,
      );
    }
  }

  return `[hotkey-actions] 动作名四处一致（含消费分支）：${JSON.stringify(rustActions)}`;
}

// ---- 脚本主体：只负责读真实文件，校验全在 checkHotkeyActions 里 ----
// main guard：自测文件 import 本模块时只拿函数、不触发这里的文件读取。
...
if (isMain) {
  ...
  const successLine = checkHotkeyActions({
    actionIdSource: readFileSync(new URL("../src-tauri/src/config.rs", import.meta.url), "utf8"),
    apiSource: readFileSync(new URL("../../client/src/lib/desktop/api.ts", import.meta.url), "utf8"),
    settingsSource: readFileSync(new URL("../../client/src/pages/settings/SettingsDesktopPage.tsx", import.meta.url), "utf8"),
    shellSource: readFileSync(new URL("../src-tauri/src/shell.rs", import.meta.url), "utf8"),
    consumerSources,
    consumerPaths: CONSUMER_FILES,
  });
