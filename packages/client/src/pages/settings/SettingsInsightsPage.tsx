  ...
  return (
    <SettingsDetailPage title="记录偏好">
      <section className="space-y-3 rounded-card border border-border bg-surface p-4">
        <div>
          <h3 className="td-text-label font-medium text-ink">新建待办默认落点</h3>
      ...
      <section className="space-y-3 rounded-card border border-border bg-surface p-4">
        <div>
          <h3 className="td-text-label font-medium text-ink">打点分类</h3>
          <p className="mt-1 td-text-caption text-ink-3">速记页和时间轴打点时，直接记录到这个子分类。</p>
        </div>
        <SelectSheet
          label="打点分类"
          placeholder="未指定"
      ...
      <section className="space-y-3 rounded-card border border-border bg-surface p-4">
        <div>
          <h3 className="td-text-label font-medium text-ink">睡眠分类</h3>
          <p className="mt-1 td-text-caption text-ink-3">用于作息、覆盖率、异常时段活动和超长记录的睡眠口径。</p>
        </div>
        <SelectSheet
          label="睡眠分类"
          placeholder="未指定"
      ...
      <section className="space-y-3 rounded-card border border-border bg-surface p-4">
        <label className="flex items-center justify-between gap-4">
          <span>
            <span className="block td-text-label font-medium text-ink">跨天记录合并展示</span>
            <span className="mt-1 block td-text-caption text-ink-3">
