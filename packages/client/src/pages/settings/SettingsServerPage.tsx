  ...
  return (
    <SettingsDetailPage title="服务器配置">
      <section className="space-y-3 rounded-card border border-border bg-surface p-4">
        <div>
          <label className="mb-1 block td-text-caption text-ink-3">API 地址</label>
          <input
            type="url"
            value={apiUrl}
        ...
        <div>
          ...
          <input
            ...
            className={inputClassName}
          />
          <p className="mt-2 td-text-caption text-warn">
            Token 会保存在本机浏览器存储中，请只在可信设备上保存服务器 Token。
          </p>
        </div>
        {error ? <p className="td-text-caption text-danger">{error}</p> : null}
        <button
          type="button"
          onClick={saveConfig}
        ...
        <div className="td-text-caption text-ink-3">新的服务器配置会用于后续同步、服务端更新和数据导出。</div>
      </section>
    </SettingsDetailPage>
  );
}
