...
export default function AgeDistributionSection({ tasks, today }: TodoStatsModuleProps) {
  ...
  return (
    <section className="rounded-card border border-border bg-surface p-4 shadow-elev1">
      <h2 className="td-text-label font-medium text-fg">存活时长分布</h2>
      <p className="mt-1 td-text-caption text-fg-muted">未完成待办从创建到现在的存活时长分布，不含重复任务。</p>
      ...
      {oldest.length > 0 ? (
        <details className="mt-3">
          <summary className="cursor-pointer td-text-caption text-fg-muted">最老 {oldest.length} 条</summary>
          <ul className="mt-2 space-y-1">
            {oldest.map((task) => (
              <li key={task.id} className="td-text-caption text-fg-muted">
