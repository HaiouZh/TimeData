  ...
  return (
    <SectionPanel title="结构诊断" eyebrow="Structure">
      {structure.current.sessionCount === 0 ? (
        <p className="td-text-body text-ink-3">本周期无足够会话用于结构诊断。</p>
      ) : (
        <>
          <div className="space-y-1 rounded-card border border-border bg-surface-elevated px-3 py-2 td-text-body text-ink">
            <div className="td-text-caption text-ink-2">
              深度 vs 浅记录{structure.excludedSleep ? "" : "（含睡眠，指定睡眠分类后更准）"}
            </div>
            <div>
              深度时间占比 <span className="text-ok">{structure.current.deepRatioPct}%</span>
              <span className="text-ink-3">（基线 {structure.baseline.deepRatioPct}%）</span>
            </div>
            <div className="td-text-caption text-ink-2">
          ...
          <div className="space-y-1 rounded-card border border-border bg-surface-elevated px-3 py-2 td-text-body text-ink">
            <div className="td-text-caption text-ink-2">碎片化（仅供观察，不报警）</div>
            <div className="td-text-caption text-ink-2">
          ...
          <div className="space-y-1 rounded-card border border-border bg-surface-elevated px-3 py-2 td-text-body text-ink">
            <div className="td-text-caption text-ink-2">投入分散度（香农熵）</div>
            <div className="td-text-caption text-ink-2">
          ...
          <div className="space-y-1">
            <div className="td-text-caption text-ink-2">占比失衡</div>
            {structure.baselineDaysWithData < INSIGHT_CONSTANTS.imbalanceMinDaysWithData ? (
              ...
              <p className="td-text-caption text-ink-3">本周期各父分类占比均在你的常态范围内。</p>
            ) : (
              <ul className="space-y-1">
                {structure.imbalances.map((item) => (
                  <li key={item.parentId} className="td-text-caption">
                    <span className="text-ink">{props.parentNameById.get(item.parentId) ?? item.parentId}</span>{" "}
                    <span className={item.direction === "high" ? "text-warn" : "text-accent"}>
