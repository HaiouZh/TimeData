  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-surface-elevated text-ink">
      <div className="border-b border-border-hairline px-4 py-3">
        <h2 className="td-text-label font-medium text-ink">目标</h2>
      </div>
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto px-3 py-3">
        {items.length === 0 ? (
          <p className="rounded-row border border-dashed border-border px-3 py-4 td-text-body text-ink-3">没有进行中的目标</p>
        ) : (
          items.map((item) => {
            ...
            return (
              ...
              >
                <span className="flex items-center justify-between gap-3">
                  <span className="min-w-0 truncate td-text-label text-ink">{item.title}</span>
                  <span className="shrink-0 td-text-caption tabular-nums text-accent">{pct}%</span>
                </span>
