...
export default function CategoryPicker({ onSelect, selectedId }: CategoryPickerProps) {
  ...
  if (parentCategories.length === 0) {
    return (
      <div className="rounded-row border border-dashed border-border p-3 td-text-body text-ink-3">
        还没有分类，请先在分类页添加。
      </div>
    );
  }
        ...
        {parentCategories.map((parent) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 rounded-pill px-3 py-2 td-text-label font-medium transition-all ${
        ...
        {(activeChildren.length > 0 ? activeChildren : activeParent ? [activeParent] : []).map((category) => {
          ...
          return (
            <button
              ...
              className={`rounded-pill px-3 py-2 td-text-label transition-all ${
