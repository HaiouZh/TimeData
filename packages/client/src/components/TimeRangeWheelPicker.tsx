...
function TimeGroup({ label, value, onChange }: { label: string; value: DateTimeValue; onChange: (value: DateTimeValue) => void }) {
  return (
...
export default function TimeRangeWheelPicker({ start, end, error, onStartChange, onEndChange }: TimeRangeWheelPickerProps) {
  return (
    <div className="rounded-2xl bg-slate-900 border border-slate-800 p-3 space-y-3">
      <div className={`rounded-xl px-3 py-2 text-center ${error ? "bg-red-950/50 text-red-300" : "bg-slate-950 text-slate-100"}`}>
