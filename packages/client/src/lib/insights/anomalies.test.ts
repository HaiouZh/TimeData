...
describe("detectAnomalies", () => {
  ...
  it("支持作息模块传入的个体化睡眠窗口", () => {
    const entries = [
      entry("late-work", "work", "2026-05-08T15:30:00.000Z", "2026-05-08T16:00:00.000Z"), // +8 23:30
      entry("night-work", "work", "2026-05-08T18:00:00.000Z", "2026-05-08T18:30:00.000Z"), // +8 02:00
    ];
    const anomalies = detectAnomalies({
      entries,
      categories,
      ...
      toDate: "2026-05-09",
      sleepCategoryId: "sleep",
      sleepWindow: { startMin: 60, endMin: 9 * 60, source: "samples" },
    });

    expect(
      anomalies.some(
        (a) => a.type === "sleepTimeActivity" && a.categoryId === "work" && a.startTime === entries[0].startTime,
      ),
    ).toBe(false);
    expect(
      anomalies.some(
        (a) => a.type === "sleepTimeActivity" && a.categoryId === "work" && a.startTime === entries[1].startTime,
      ),
    ).toBe(true);
    expect(anomalies.find((a) => a.type === "sleepTimeActivity")?.message).toContain("01:00~09:00");
  });
  ...
  it("空 entries 时范围内每天报 unrecordedDay，其余类不报", () => {
    ...
    expect(anomalies.filter((a) => a.type === "unrecordedDay").map((a) => a.date)).toEqual([
      "2026-05-09",
      "2026-05-08",
    ]);
