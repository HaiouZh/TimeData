 ...
 * `bottomBarHeightPx`）与底部导航占位（`navOffsetPx`），本函数只负责把它们与键盘遮挡量
 * （`keyboardHeightPx`，见 `useKeyboardHeight`——是「键盘还挡着多少、JS 还要让开多少」，
 * 壳自己让过位时为 0，不是键盘自身高度）合成为一个基础量（px，不含 safe-area）。
 *
