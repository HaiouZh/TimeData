...
// 跳变补偿的时长与曲线由调用方（KeyboardDock）把 useKeyboardMotion 的同一份参数传进来：
// 抬升过渡与跳变补偿同曲线同时长，两种运动叠加/衔接时不出现速度断层。此前这里与 index.css 各写一份
// 常量、要求人肉同步，R7 起单一来源在 lib/keyboard/keyboardMotionPrefs.ts。
 ...
 * 「从跳变量滑回 0」的**附加**动画——首帧视觉位置不变，随后按传入时长滑到新位置。
 *
 ...
 */
export function useShellResizeGlide(ref: RefObject<HTMLElement | null>, durationMs: number, easing: string): void {
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      el.animate([{ transform: `translateY(${delta}px)` }, { transform: "translateY(0px)" }], {
        duration: durationMs,
        easing,
        composite: "add",
      });
