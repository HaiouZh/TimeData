...
function replaceServerData(
  ...
  const insertTask = db.prepare(`
    INSERT INTO tasks (id, title, done, recurrence, last_done_at, start_at, sort_order, scheduled_at, parent_id, completed_count, turn, turn_at, completed_at, tags, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
