...
import { type SyncLogDbRow, parseSyncLogDetail, readNumberField } from "./_helpers.js";
...
sync.get("/", (c) => {
  ...
  for (const row of rows) {
    ...
    for (const outcome of items) {
      ...
      const tableName =
        outcome.tableName === "categories" || outcome.tableName === "time_entries" ? outcome.tableName : null;
