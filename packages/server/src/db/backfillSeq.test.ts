...
beforeEach(() => {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS tasks (id TEXT PRIMARY KEY, title TEXT NOT NULL, done INTEGER NOT NULL DEFAULT 0, recurrence TEXT, last_done_at TEXT, start_at TEXT, sort_order INTEGER NOT NULL DEFAULT 0, scheduled_at TEXT, parent_id TEXT, completed_count INTEGER NOT NULL DEFAULT 0, turn TEXT, turn_at TEXT, completed_at TEXT, tags TEXT NOT NULL DEFAULT '[]', session_id TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS sessions (id TEXT PRIMARY KEY, started_at TEXT NOT NULL, ended_at TEXT, note TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
...
describe("backfillMissingSeq", () => {
  it("records a create seq for business rows missing one (incl. seedless default categories)", () => {
    ...
    db.prepare("INSERT INTO sessions (id, started_at, created_at, updated_at) VALUES ('session-1', 't', 't', 't')").run();
    ...
    expect(seqCount("sessions", "session-1")).toBe(1);
